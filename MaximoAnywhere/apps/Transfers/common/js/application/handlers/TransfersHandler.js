...
/**@memberOf application.handlers.TransfersHandler */
		...
		resetAdditionalItemFields : function(eventContext){
			...
			eventContext.application['application.handlers.TransfersAvailableItemsHandler'].clearAdditionalUsage(eventContext);
...
//							eventContext.application.addResource(shipmentSet);
		...
		initiateTransfer : function(eventContext, newStatus){
			...
			var transfersReservedHeader  = eventContext.application.getResource("transfersReservedHeader").getCurrentRecord();
...
//					invuse.shipmentline = [];
		...
		transitsBackToTransferPlannedSearchView : function(eventContext) {
			...
			//#region Loc-In: clear Reserved Header
			eventContext.application['application.handlers.TransfersHandler'].clearReservedHeader(eventContext);
			//#endregion Loc-Out: clear Reserved Header
			eventContext.ui.hideCurrentView(PlatformConstants.CLEANUP);
		},
...
//			
		...
		selectFromBinLot: function(eventContext) {
			...
			if(previousView == "Transfers.InvreserveListView"){
				...
				eventContext.application['application.handlers.TransferAvailableItemsHandler'].selectFromBinLot(eventContext);
			}
...
//			console.log("CLEAR rotating asset selection");
		...
		clearReservedHeader : function(eventContext){
			var reservedHeaderResource = CommonHandler._getAdditionalResource(eventContext, 'transfersReservedHeader');
		...
		selecToBinLotByPreviousView: function(eventContext){
			...
			if(previousView == "Transfers.AdditionalItemsDetailsView"){
				eventContext.application['application.handlers.TransfersAvailableItemsHandler'].selectToBinLot(eventContext);
			} else if (previousView == "Transfers.ShipmentItemsListView"){
				eventContext.application['application.handlers.ReceiveShipmentHandler'].selectToBinLot(eventContext);
			}
		},
		...
		clearToBinLotSelectionByPreviousView : function(eventContext){			
			var previousView = WL.application.ui.viewHistory[WL.application.ui.viewHistory.length-2].id;
			if(previousView == "Transfers.AdditionalItemsDetailsView"){
				eventContext.application['application.handlers.TransfersAvailableItemsHandler'].clearToBinLotSelection(eventContext);
			} else if (previousView == "Transfers.ShipmentItemsListView"){
				eventContext.application['application.handlers.ReceiveShipmentHandler'].clearToBinLotSelection(eventContext);
			}			
		},
