...
define("application/handlers/POAttachmentHandler", 
	   [ "dojo/_base/declare",
	     "dojo/_base/lang",
...
/**@memberOf application.handlers.WOAttachmentHandler */
		...
		"displayLocal": function(eventContext) {
			...
			if (downloadCompleted) {
				...
				//eventContext.setDisplay(false);
				eventContext.setImage('msg_downloadIncomplete_small.svg');
			}
		...
		initlializePOResource: function(eventContext){
			/* var filter = [];
			...
			var promise =  ModelService.filtered('poResource', PlatformConstants.SEARCH_RESULT_QUERYBASE, filter, 100, true, true, null, false);
			promise.then(function(set){
				
				ModelService.clearSearchResult(set);
				set.resourceID = 'poResource';
