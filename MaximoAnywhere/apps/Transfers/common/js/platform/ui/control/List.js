...
define("platform/ui/control/List", [
    ...
    return declare(ContainerControlBase, {
        ...
        constructor: function (options) {
            ...
            this.moveTo =
                this.transitionTo && this.transitionTo.length > 0 ? "#" + this.transitionTo : "";
            if (!options || !options.transition) {
                this.transition = "slide";
            }
            this.translatedSortLabel = MessageService.createDynamicMessage(
                "sortby",
        ...
        applyDatabaseFilterAndSearch: function () {
            ...
            var isExactMatch =
                currentModelDataSet.lookupFilter &&
                currentModelDataSet.lookupFilterExactMatch === false
                    ? false
                    : true;
            ...
            if (query && Object.keys(query[0]).length) {
                return ModelService.filtered(
                    currentModelDataSet.getResourceName(),
                    currentModelDataSet.getQueryBase(),
                    query,
                    null,
                    false,
                    isExactMatch
        ...
        build: function () {
            ...
            if (!this.initialized) {
                ...
                if (this.viewControl) {
                    //some lists (long press menu) don't have a specific view
            ...
            var sortIndexSet = this.sortIndexResource
                .clearFilterAndSort()
                .filter("listid == '" + this.id + "'");
            ...
            if (this.showHeader) {
                ...
                if (sortable && !this.searchable && !this.hideSort) {
                    this.sortLabel = new Link({
                        ...
                        label: "",
                        labelClassName:
                            (this.sortOptions.length <= 1 ? "non" : "") +
                            "sortLabel textappearance-medium",
                ...
                if (this.searchable) {
                    ...
                    this.hideErrorIcons = new Boolean(
                        onErrorPage || (resource != null && resource.getParent() != null)
                    );
                ...
                if (this.errorCountLabel) {
                    ...
                    if (!this.errorWatch) {
                        this.errorWatch = topic.subscribe(
                            PlatformConstants.DATA_REFRESH_TOPIC + "/" + resource.getResourceName(),
                ...
                domStyle.set(
                    this.countLabel.labelElement.domNode,
                    "background-repeat",
                    "no-repeat"
                );
                ...
                if (this.errorCountLabel && this.errorCountLabel.baseWidget) {
                    this.addHandler(
                        on(
                            this.errorCountLabel.baseWidget.domNode,
                            touch.press,
                            lang.hitch(this, function () {
                                var queries = this.viewControl.queries.children;
                                for (var index = 0; index < queries.length; index++) {
                                    if (
                                        queries[index].queryBase ==
                                        PlatformConstants.ERRORED_QUERYBASE
                                    ) {
                ...
                if (sortable && this.sortLabel != null) {
                    ...
                    domAttr.set(
                        this.sortLabel.baseWidget.domNode,
                        "aria-label",
                        this.sortLabel.baseWidget.domNode.textContent
                    );
            ...
            if (!listResourcePromise) {
                ...
                if (this.getResource() && this.showHeader) {
                    ...
                    this.getResource()
                        .getListCount()
                        .then(function (count) {
                            if (self.countLabel) self.countLabel.setLabel(count);
                        });
                }
            ...
            if (this.baseWidget) this.baseWidget.addChild(this.listWidget);
            ...
            query(".mblListItemRightIcon", createdBaseWidget.domNode).forEach(function (
                itemIcon,
                index,
                arr
            ) {
                ...
                domAttr.set(
                    itemIcon.firstElementChild,
                    "alt",
                    MessageService.createStaticMessage("details").getMessage()
                );
        ...
        _setupSearchCriteria: function (metadata) {
            ...
            var searchSet = this.searchResource
                .clearFilterAndSort()
                .filter("listid == '" + this.id + "'");
            ...
            if (this.searchable) {
                ...
                this["canSearchStore"] = !array.some(
                    this.searchAttributes.children,
                    function (child) {
                        if (child.name) {
                ...
                ConnectivityChecker.checkConnectivityAvailable().then(function (hasConnection) {
                    ...
                    if (self.reloadButton && hasConnection) {
                        ...
                        if (resource) {
                            showReload = resource
                                .getMetadata()
                                .dataPastFetchLimit(resource.getQueryBase());
                        }
        ...
        checkForErrors: function () {
            ...
            if (
                !resourceMetadata.additionalData &&
                !resourceMetadata.isSystem &&
                !resourceMetadata.isLocal()
            ) {
                if (resource._queryBaseName == PlatformConstants.ERRORED_QUERYBASE) {
                    ...
                    ModelService.countErrored(resourceMetadata.getResourceName()).then(function (
                        count
                    ) {
            ...
            if (this.hasSorting() || this.showHeader) {
                ...
                array.forEach(
                    ...
                    lang.hitch(this, function (action, index) {
                        this.sortOptionMenuItems.push({
                            label: action.label.getMessage(),
                            listControl: this,
                            index: index,
                            eventHandlers: [
                                {
        ...
        buildListWidget: function (isEmpty) {
            ...
            requires.push(
                this.mixinclass == "SpecificationStoreList"
                    ? SpecificationStoreList
                    : EdgeToEdgeStoreList
            );
            ...
            if (this.badKeys) {
                this.logger.log(
                    "[Warning] While building list " +
                        this.id +
                        ". Keys defined in resource do not provide for uniqueness across rows.",
                    1
                );
            }
            if (this.viewControl) {
            ...
            if (listResource && listResource.getMetadata().serverOnlyMode) {
                ...
                ConnectivityChecker.checkConnectivityAvailable().then(function (
                    ...
                    if (isConnectionAvailable) {
                        ...
                        self.application.showMessage(
                            MessageService.createStaticMessage("sortFailedConnection").getMessage()
                        );
                    }
                });
            } else {
            ...
            if (this.hasSorting()) {
                ...
                if (sortParams.length > 0) {
                    ...
                    if (listResource.count() > 0) {
                        if (
                            listResource._queryBaseName !=
                                PlatformConstants.SEARCH_RESULT_QUERYBASE &&
                            !listResource.isFiltered() &&
                            (!listResource.isDirty() || listResource._getSaveStage() == 0) &&
                            !listResource.getParent() &&
                            !this.ui.movingBack
                        ) {
                            // don't sort if it is a child and it is moving back, in a pagination data set it will cause the index to be reset (0), loosing the current position
                            self.application.showBusy();
                            ModelService.sort(listResource, this.concatSortParams(sortParams)).then(
                                function (dataSet) {
                                    var resourceID = dataSet.getParent()
                                        ? dataSet.getParent().getOwner().name +
                                          "." +
                                          dataSet.getRelationNameWithParent()
                                        : listResource.getResourceName();
                            ...
                            if (!cleared) {
                                ...
                                if (
                                    parentView &&
                                    parentView.filterClass &&
                                    parentView.filterMethod
                                ) {
                                    listResource.clearFilterAndSort();
                                    this.applyFilterToSet();
                                }
        ...
        search: function () {
            ...
            this.logger.log(
                "[List search] One or more search attributes for the list " +
                    this.artifactId +
                    " are not defined as indexes on the resource " +
                    this.getResource().getResourceName() +
                    " so search will only be on the current resource pages in memory"
            );
        ...
        splitPaneModeSaveAndReload: function () {
            var editViewControl = this.ui.getViewFromId(
                this.ui.viewHistory[this.ui.viewHistory.length - 1].id
            );
            if (editViewControl.validate() && this.ui.dialogStack.length === 0) {
                this.ui.application.showBusy();
                var resourceName = this.getResource().getResourceName();
                var queryBase = this.getResource().getQueryBase();
                var self = this;
                ModelService.save(this.getResource())
                    .then(function (s) {
                        ModelService.allCached(resourceName, queryBase)
                            .then(function (dataSet) {
                                dataSet.resourceID = resourceName;
                                self.ui.application.addResource(dataSet);
                                self.setMyResourceObject(dataSet);
                                dataSet.setCurrentIndex(0);
                                self.refresh();

                                editViewControl.refresh();
                                self.ui.application.hideBusy();
                            })
                            .otherwise(function (error) {
                                Logger.trace("SplitPane list reload. All Cache failed ", error);
                                self.ui.application.hideBusy();
                            });
                    })
                    .otherwise(function (error) {
                        Logger.trace("SplitPane list reload. Save failed ", error);
                        self.ui.application.hideBusy();
                    });
            }
        },
        ...
        postCreate: function () {
            ...
            if (this.refreshButton && !this.refreshButton.clickHandler) {
                this.refreshButton.clickHandler = on(
                    this.refreshButton.baseWidget.domNode,
                    tap,
                    lang.hitch(this, function () {
                        dijitFocus.focus(this.refreshButton.baseWidget.domNode);
                        if (this.ui.combinedViews) {
                            this.splitPaneModeSaveAndReload();
                        } else {
                            this.refresh();
                        }
                    })
            ...
            if (this.reloadButton && !this.reloadButton.clickHandler) {
                this.reloadButton.clickHandler = on(
                    this.reloadButton.baseWidget.domNode,
                    tap,
                    lang.hitch(this, function () {
        ...
        reload: function () {
            var resource = this.getResource();
            var self = this;
            Logger.trace("[List.reload] - Reloading list: " + this.getId());
            ConnectivityChecker.checkConnectivityAvailable().then(function (hasConnection) {
                if (hasConnection) {
                    self.ui.show("Platform.ReloadingCurrentWorklist");
                    var progressRecord = self.application
                        .getResource("PlatformProgressResource")
                        .getRecordAt(0);
                    progressRecord.set(
                        "progressMsg",
                        MessageService.createResolvedMessage("reloadingWorkListProgress", ["0"])
                    );
                    Logger.trace(
                        "[List.reload] - calling ModelService.reloadAllPages for resource: " +
                            resource.name
                    );
                    ModelService.reloadAllPages(resource.name, resource.getQueryBase()).then(
                        function (resultSet) {
                            Logger.trace(
                                "[List.reload] - ModelService.reloadAllPages returned sucessfully for resource: " +
                                    resource.name
                            );
                            ...
                            self.refresh();
                            Logger.trace("[List.reload] - hiding progress dialog after success");
                            self.ui.hideCurrentDialog();
                        },
                        function (error) {
                            Logger.trace(
                                "[List.reload] - ModelService.reloadAllPages returned with error for resource: " +
                                    resource.name
                            );
                            self.logger.error(
                                "Error reloading resource " + resource.getResourceName()
                            );
                            ...
                            Logger.trace("[List.reload] - hiding progress dialog after error");
                            self.ui.hideCurrentDialog();
                        },
                        function (progress) {
                            progressRecord.set(
                                "progressMsg",
                                MessageService.createResolvedMessage("reloadingWorkListProgress", [
                                    progress,
                                ])
                            );
                        }
                    ...
                    var message = MessageService.createStaticMessage(
                        "reloadFailedNoConnectivity"
                    ).getMessage();
        ...
        refresh: function (preventSort) {
            Logger.trace("[List.refresh] - refresh list: " + this.artifactId);
            this._syncWithAppResource();
            ...
            this.destroyList();
            this.buildListWidget();
            if (this.listWidget) this.baseWidget.addChild(this.listWidget);
            domStyle.set(this.listWidget.domNode, "display", "inline");
            this.buildSortMenu();
            var resource = this.getResource();
            var self = this;
            ...
            this.application.hideBusy();
            Logger.trace("[List.refresh] - refresh list: " + this.artifactId + " - COMPLETED");
        },

        makeCollapsible: function () {
            var self = this;
            this.addHandler(
                on(self.header.baseWidget, "click", function (e) {
        ...
        _showCountLableWaitingIcon: function () {
            ...
            domStyle.set(
                this.countLabel.labelElement.domNode,
                "background-image",
                "url(idx/mobile/themes/common/idx/images/loadingIndicatorWhite.gif)"
            );
        },
        ...
        fixTransitionIcons: function () {
            var list = this;
            if (list.listWidget) {
                query(".mblListItemRightIcon", list.listWidget.domNode).forEach(function (
                    itemIcon,
                    index,
                    arr
                ) {
