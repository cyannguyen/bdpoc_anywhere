...
function dojoInit(has, instrumentation, Deferred, aspect, BusyIndicator, Logger, MessageService) {
    ...
    aspect.before(Deferred, "instrumentRejected", function(error, handled, rejection, deferred) {
        try {
            ...
            if (BusyIndicator.ignoreTimeout !== true && Object.keys(active_timers).length == 0) {
    ...
    require(["dojo",
        ...
        function(dojo, Main, Logger) {
            ...
            dojo.ready(function() {
                ...
                document.addEventListener("resume", onResumeReportConnection, false);

                function onResumeReportConnection() {
...
document.addEventListener("anywhereready", function() {
    //wlCommonInit();
