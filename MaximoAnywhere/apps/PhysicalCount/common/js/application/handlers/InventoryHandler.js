    ...
    return declare([ApplicationHandlerBase], {
        ...
        processCount: function (eventContext) {
            ...
            ModelService.fetchFromServer("invbalance", querybase).then(function (serverCheck) {
                ...
                ModelService.save(invBalanceSet)
                    .then(function () {
                        CommunicationManager.checkConnectivityAvailable().then(function (
                            hasConnectivity
                        ) {
                            if (hasConnectivity) {
                                if (self.needsQueryReset) {
                                    ...
                                    ModelService.refreshDataForWorkListIfPossible(
                                        ...
                                        invBalanceSet.getPageCountInMemory(),
                                        true,
                                        true
                                    ).always(afterDataFetch);
                                }
        ...
        isBlindCount: function (eventContext) {
            var invBalance = CommonHandler._getAdditionalResource(
                eventContext,
                "invbalance"
            ).getCurrentRecord();
            var querybase = invBalance.getOwner().getQueryBase();
            ...
            if (
                !SystemProperties.getProperty("si.adminmode") ||
                SystemProperties.getProperty("si.adminmode") == "false"
            ) {
                var blindCountResource =
                    eventContext.application.getResource("blindCountQueryBases");
        ...
        populateSearch: function (eventContext) {
            ...
            if (eventContext.application.ui.performSearch) {
                if (eventContext.application.getResource("searchItems") == null) {
                    //TODO:  might be nice to still open to last search
                ...
                if (view.queries && view.queries != null) {
                    while (i < view.queries.children.length) {
                        if (
                            view.queries.children[i].queryBase ==
                            PlatformConstants.SEARCH_RESULT_QUERYBASE
                        ) {
                            indexOfSearch = i;
                ...
                //In-Loc: bin range
                if (search.binfrom && search.binto) {
                    //In-Tuan: fix filter for bin range
                    // var binFrom = search.binfrom;
                    // var binTo = search.binto;
                    ...
                    // 	filteredItems++;
                    // }
                    var binFromObj = this.checkBinRange(search.binfrom);
                    var binToObj = this.checkBinRange(search.binto);
                    var fromValue = parseInt(binFromObj.suffix);
                    var toValue = parseInt(binToObj.suffix);
                    if (toValue - fromValue >= 10000) {
                        eventContext.application.showMessage("Bin Range too large");
                        return;
                    }
                    for (var i = fromValue; i <= toValue; ++i) {
                        bigFilter.push({ binnum: binFromObj.prefix + i });
                    }
                    filteredItems++;
                    //Out-Tuan: fix filter for bin range
                }
                //Out-Loc: bin range
                ...
                var queryBaseForSearch =
                    view.queries.children[eventContext.application.ui.savedQueryIndex].queryBase;
        ...
        initDetail: function (eventContext) {
            var currentQueryBase =
                eventContext.application.getResource("invbalance")._queryBaseName;
        ...
        initSearchData: function (eventContext) {
            ...
            eventContext.application.ui.savedQueryIndex =
                eventContext.application.ui.getViewFromId("Inventory.ItemsView").queryBaseIndex;
        },
        ...
        populateItemDetail: function (eventContext) {
            ...
            eventContext
                .getResource()
                .getRecordAt(0)
                .set("description", invbalance.get("description"));
            ...
            eventContext
                .getResource()
                .getRecordAt(0)
                .set("lastcountdate", invbalance.get("lastcountdate"));
        ...
        setStoreRoomTitle: function (eventContext, storeRoomTitle) {
            /* #region Tuan-in: remove unuse resource additionalstoreroom to prevent bug when users do not have lookup data */
            ...
            /* #endregion Tuan-out: remove unuse resource additionalstoreroom to prevent bug when users do not have lookup data */
            storeRoom = null;
            localRecord = eventContext.application
                .getResource("storeRoomResource")
                .createNewRecord();
            ...
            var storeroomSetting = CommonHandler._getAdditionalResource(
                eventContext,
        ...
        reloadDefaultStoreRoom: function (eventContext) {
            ...
            if (storeRoom.getCurrentRecord().get("location") == null) {
                var cuurentChooseStoreRoom = eventContext.application
                    .getResource("chooseStoreRoomResource")
                    .getCurrentRecord();
        ...
        loadInvbalanceList: function (eventContext) {
            ...
            //var physcntdateFormat = FormatterService.toDisplayableValue(dateTimeISOFormatter.fromISOString(physcntdate), "dateTime", this.userInterface.application.getUserLocale());
            var row3 =
                "Unit: " + currentRecord.get("issueunit") + ", Last Count Date: " + physcntdate; // physcntdateFormat;
        ...
        checkCanCount: function (eventContext) {
            ...
            var additionaluserinfo = CommonHandler._getAdditionalResource(
                eventContext,
                "additionaluserinfo"
            );
        ...
        checkEmptyStoreroom: function (eventContext) {
            ...
            var locationPromise = ModelService.filtered(
                "storeroom",
                PlatformConstants.SEARCH_RESULT_QUERYBASE,
                filter,
                1000,
            ...
            var self = this;
            locationPromise.then(function (arrayStoreRoom) {
                if (arrayStoreRoom && arrayStoreRoom.count() > 1) {
                    var defaultStoreroom = arrayStoreRoom.data[1].location;
                    self.setStoreRoomTitle(eventContext, defaultStoreroom.toUpperCase());
                    self.loadInvbalanceByQueryBase(eventContext);
                }
            });
        },
        /* #endregion Tuan-out: add default storeroom */

        validateChooseStoreRoom: function (eventContext) {
            //eventContext.application.showMessage('AAA');
            var res = false;
            var arrayStoreRoom = CommonHandler._getAdditionalResource(
                eventContext,
                "additionalstoreroom"
            );
        ...
        clearChooseStoreRoom: function (eventContext) {
            var chooseStoreRoom = eventContext.application
                .getResource("chooseStoreRoomResource")
                .getCurrentRecord();
        ...
        showFilteredList: function (eventContext, queryBase) {
            ...
            if (currentStoreRoom) {
                ...
                ModelService.scan("invbalance", querybase, filter)
        ...
        loadOnlineInvbalance: function (eventContext) {
            ...
            if (currentStoreRoom && !this.isFiltered) {
                ...
                ModelService.fetchFromServer("invbalance", querybase).then(function (
                    hasConnectivity
                ) {
                    eventContext.application.showBusy();
                    if (hasConnectivity) {
                        ...
                        ModelService.filtered(
                            ...
                            .then(function (dataSet) {
                                ...
                                if (dataSet.count() > 0) {
                                    ...
                                    promise.then(function (invbalanceSet) {
                                        ModelService.clearSearchResult(invbalanceSet).then(
                                            function () {
                                                if (invbalanceSet.count() > 0) {
                                                    /* arrayUtil.forEach(invbalanceSet.data, function(data){
											data.setQueryBase("__search_result__");									
                                                    ...
                                                    ModelService.save(invbalanceSet).then(
                                                        function () {
                                                            invbalanceSet.setCurrentIndex(0);
                                                        }
                                                    );
                                                    ...
                                                    //ModelService.clearSearchResult(invbalanceSet);
                                                    self.loadLocalInvbalanceByStoreRoom(
                                                        eventContext
                                                    );
                                                    eventContext.application.hideBusy();
                                                }
        ...
        handleConditionBinnum: function (eventContext) {
            var currentSearchItems = CommonHandler._getAdditionalResource(
                eventContext,
                "searchItems"
            ).getCurrentRecord();
        ...
        _showSearchFailedMessageNoConnectivity: function (eventContext) {
            eventContext.application.showMessage(
                MessageService.createStaticMessage("downloadFailedNoConnectivity").getMessage()
            );
            if (
                eventContext.application.ui.getViewFromId("Inventory.ItemsView").lists[0].baseWidget
            ) {
        ...
        initializeBarcodeScanner: function (eventContext) {
            ...
            if (this.scanHandler) {
                ...
                this.application["platform.handlers.CodeScannerHandler"].subHandler =
                    this.application["application.handlers.CodeScannerHandlerExt"];
        ...
        discardSummaryView: function (eventContext) {
            ...
            var currentStoreRoom = eventContext.application
                .getResource("storeRoomResource")
                .getCurrentRecord();
            // Tuan-in: not reload list search results
            if (currentStoreRoom.get("location") != null && savedQueryIndex == 0) {
                // if (currentStoreRoom.get("location") != null) {
                // Tuan-out: not reload list search results
                var self = this;
                // Tuan-in: fix changeQueryBaseIndex
                    ...
                    if (!hasConnectivity) {
                        ...
                        promise.then(function () {
                            CommunicationManager.checkConnectivityAvailable().then(function (
                                ...
                                if (!hasConnectivity) {
                                    var invbalanceSet = CommonHandler._getAdditionalResource(
                                        eventContext,
                                        "invbalance"
                                    );
        ...
        filterItemsForLookup: function (eventContext) {
            ...
            var searchRecord = CommonHandler._getAdditionalResource(
                eventContext,
                "searchItems"
            ).getCurrentRecord();
        ...
        filterStoreroomForLookup: function (eventContext) {
            ...
            if (itemnum) {
                CommonHandler._filterAdditionalInventoryByItemnum(
                    eventContext,
                    additionalInventory,
                    itemnum.toUpperCase()
                );
        ...
        setOriginatingQuerybase: function (eventContext, querybase) {
            if (querybase) {
                ...
                originatingQuerybase =
                    eventContext.application.ui.getCurrentViewControl().queryBase;
            }
        },
