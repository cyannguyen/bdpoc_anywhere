...
define("platform/ui/control/Map",
		...
		WorkOfflineMapDataLoader, Logger, ModelService, LayoutWidgetBase, domConstruct, domClass) {
...
/**@memberOf platform.ui.control.Map */
			build : function() {
				...
				this.popSubscription = topic.subscribe('dialogpop', function(published){
					if (!mapControl.mapShown && published['count'] == 0){
						//If Spatial is installed, we cannot can showMap again if offline map is shown
						...
						var isOfflineSpatial = false;
						if (mobileMaximoSpatial != null) {
							isOfflineSpatial = !mobileMaximoSpatial.showingOnlineMap;
						}
						if (mapControl.hiddenAsPortrait == mapControl.isPortrait || isOfflineSpatial){
			...
			showMap : function(useCurrentIndex, showWOPanel) {				
			    ...
			    if (provider != MapProperties.ANYWHERE_PROVIDER_MAPMANAGER  && this.viewControl && this.viewControl.overFlowButton) {
				...
				if (showWOPanel == null || showWOPanel == true) {
					...
					//force map to get new container width
					domStyle.set(this.baseWidget.domNode, 'width',  "100%");
				...
				if (this.mapFactoryHelper && this.abstractMap) {					
					...
					this._setupMap(useCurrentIndex, true, showWOPanel);
				}
				else {
					var mapControl = this;
				    ...
				    when(this.mapFactoryHelper.getMap(), function(map) {
						...
						mapControl._setupMap(useCurrentIndex, false, showWOPanel);
					}, function(message){
						mapControl.platformMapMarkerInfo.set('currentMarker', '');
						if (message){
							var messageText = null;
							if (message.messageId) {
								messageText = MessageService.createResolvedMessage(message.messageId, message.parameters);
							} else {
								messageText = message;
							}
							domAttr.set(mapControl.baseWidget.domNode, "innerHTML", '<p class="maperror">'+ messageText +'</p>');
						}
					});				
				}
			},
			
			_setupMarkers : function(useCurrentIndex, showWOPanel) {
				...
				when (this.abstractMap.getResourceIndexesFromMarkers(this.mainLayerId), function(indexArray) {
					...
					setTimeout(function(){
						mapControl._fixMap(mapControl, showWOPanel);
						dojo.publish('_mapDone', []);
					}, 1000);
				});
           ...
           _setupMap : function(useCurrentIndex, checkTokenMXSpatial, showWOPanel) {
				...
				var that = this;
				var id = "map-error"+WL.application.ui.getCurrentView().id;
				domConstruct.destroy(id);
				var setupResource = function(set){
					...
					// refreshing the viewControl fix the problems
					// although is not the most efficient solution
					var originalLength = that.getResource().data.length;
					if (set.data.length > originalLength) {
							...
							if(!that._isFromMultipleRecords(that)){
									that.abstractMap.addLayer(thatResource.data, that.mainLayerId, that.getResource().getCurrentIndex()).then(function() {
	                                    mapControl.abstractMap.getAllDirections(mapControl.mainLayerId).then(function(){
	                                         mapControl._setupMarkers(useCurrentIndex, showWOPanel);
			...
			_showMapDetailError: function(errorMessage, showWOPanel){				
				if(errorMessage && errorMessage.messageId && this.viewControl && this.viewControl.header && this.viewControl.header.domNode){
					...
					if (showWOPanel == null || showWOPanel == true) {
						...
						var msgHtml = null;
						var cssList = "map-error-nocoordinate";
						if(errorMessage.messageId === "noCoordinates"){
							this.hasWOCoordinates = false;
	                  		
							msgHtml = '<p class="mapDetailsNoMarkers">'+ msg+'</p>';
							cssList += " mapErrorNoMarkers";
						}
						else{
							msgHtml = '<p class="maperror">'+ msg+'</p>';
						}  
						
						if (!this.isPortrait) {
							cssList += " error-landscape";
						} 
						var id = "map-error"+currentViewId;
						domConstruct.destroy(id);
						var errorDiv = domConstruct.create('div', { innerHTML: msgHtml});
						errorDiv.setAttribute("id",id);
						errorDiv.setAttribute("class",cssList);
						var mapDiv = dojo.byId(currentViewId);
						domConstruct.place(errorDiv, mapDiv, "last");
					}
			...
			loadOfflineData : function(dataSet, progressResource, deferred){
				...
				WorkOfflineMapDataLoader.loadMapData("esri", dataSet, this.mapOptions)
					...
					.otherwise(function(error) {
						...
						Logger.error("[platform/ui/control/Map]: " + message);
						var errorJson = {"message": message, "isWarning": true};
					    _deferred.reject(errorJson);
						progressResource.set('progressMsg', message);
					});
			},
			 ...
			 */
			_fixPortraitMap: function(mapControl, currentTable, showWOPanel){
	            ...
	            var width = mapControl.portraitContainerSize.w;
	            var height = mapControl.portraitContainerSize.h;
	            var provider = mapControl.mapOptions.provider.toLowerCase();
			    if (provider == MapProperties.ANYWHERE_PROVIDER_MAPMANAGER) {
			    	 var spatialMapHandler = WL.application["platform.handlers.spatial.SpatialMapHandler"];
					mobileMaximoSpatial = spatialMapHandler['mobileMaximoSpatial'];
					
					if (!mobileMaximoSpatial.showingWODetailPanel) {
						height = dojo.window.getBox().h;
					}
				} 
	           
				//fix size problem
	            col_map.getElementsByClassName("dijitContentPane")[0].setAttribute("style","width:"+width+"px; height:"+height+"px");
			 ...
			 * based on MapView.js, we reconstruct the map when it was built wrong
			 */
			_fixLandscapeMap: function(mapControl, currentTable, showWOPanel){
				var table = domConstruct.create('table');
	            table.style.width = '100%';
	            domClass.add(table, 'layout MapView ');
				...
				if(divTable){
					...
					if(mytable){
						...
						if((needFixSize && mapControl.isPortrait) || 
							...
							newTable = mapControl._fixPortraitMap(mapControl, mytable, showWOPanel);
						}
							...
							newTable = mapControl._fixLandscapeMap(mapControl, mytable, showWOPanel);
						}
						if(newTable){
						...
						if (emitEvent == null || emitEvent == true) {
							dojo.publish('_fixMapDone', {'showWOPanel': showWOPanel});
						}
