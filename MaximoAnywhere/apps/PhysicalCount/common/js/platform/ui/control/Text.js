    ...
    return declare([ContainerControlBase, StatefulControlMixin, BoundControlMixin], {
        ...
        build: function () {
            ...
            this.textWidget = this.createWidget(tb, {
                ...
                'aria-label': this.label?this.label.textMsg : this.getPropertyString('placeHolder'),
                style: this.style,
                size: fldLength <= 5? 12: fldLength,//Setting a cutoff to avoid text clipping,
                maxLength: fldLength,
                'class': this.classPrefix + ((this['cssClass']) ? this['cssClass'] : '') + (runtimeRequired ? ' requiredText' : '')
            });
			try{
            	if(flLen){
                	this.textWidget.domNode.maxLength = flLen;
                	this.textWidget.domNode.size = flLen;
            	}
            }catch(err){}
