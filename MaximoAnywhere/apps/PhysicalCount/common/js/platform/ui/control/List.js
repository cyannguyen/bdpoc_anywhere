...
/**@memberOf platform.ui.control.List */
            ...
            if (this.hasSorting()) {
                ...
                if (sortParams.length > 0) {
                    ...
                    if (listResource.count() > 0) {
                    	if (listResource._queryBaseName != PlatformConstants.SEARCH_RESULT_QUERYBASE && !listResource.isFiltered() && 
                        		(!listResource.isDirty() || listResource._getSaveStage() == 0) &&
                        		(!listResource.getParent() && !this.ui.movingBack)) { // don't sort if it is a child and it is moving back, in a pagination data set it will cause the index to be reset (0), loosing the current position
        ...
        splitPaneModeSaveAndReload: function(){
        	...
        	var editViewControl = this.ui.getViewFromId(this.ui.viewHistory[this.ui.viewHistory.length - 1].id)
        	if(editViewControl.validate() && this.ui.dialogStack.length === 0){
				...
				var self = this;
				ModelService.save(this.getResource()).then(function(s){
        ...
        reload: function () {
            ...
            ConnectivityChecker.checkConnectivityAvailable().then(function (hasConnection) {
                if (hasConnection) {
                    ...
                    Logger.trace("[List.reload] - calling ModelService.reloadAllPages for resource: " + resource.name);
                    ModelService.reloadAllPages(resource.name, resource.getQueryBase()).then(function (resultSet) {
                        Logger.trace("[List.reload] - ModelService.reloadAllPages returned sucessfully for resource: " + resource.name);
                        ...
                        Logger.trace("[List.reload] - ModelService.reloadAllPages returned with error for resource: " + resource.name);
