...
function(declare, ContainerControlBase, Menu, ContentPane, Form, domConstruct, Stateful, getStateful, lang, array, on, Logger, LabelManager, registry,
	...
	return declare(ContainerControlBase, {
	    ...
	    suppressBack : false,
	    splitPane: {
	    	leftPaneRootView: null,
	    	rightPaneRootView: null,
	    	historyStack: []
	    },
	    ...
	    build : function(ignr_cmbnd_view) { // not deferred
		    // summary:
		    // Build the control
		    //
		    // description:
		    ...
		    if(this.application.combinedViews){ //is it allowed on this app?
			    ...
			    if(screenSize.width>5.1 && screenSize.height > 5.1 && this.application.features['splitpane.enabled']){
			    //if(layoutWidth=='large' || layoutWidth=='xlarge' || layoutHeight == 'large' || layoutHeight=='xlarge'){
		    		this._configureCombinedView(ignr_cmbnd_view);
		    	}
	    ...
	    _configureCombinedView : function(ignr_cmbnd_view){
    		...
    		this._setRootViewsForCombinedView(this.mainSplitter.children[0].id, this.mainSplitter.children[1].id);
		},

		_setRootViewsForCombinedView: function(leftPaneView, rightPaneView){
			this.splitPane.leftPaneRootView = leftPaneView;
			this.splitPane.rightPaneRootView  = rightPaneView;
		},
	    ...
	    back : function() {
        	...
        	Logger.trace("[UserInterface - back]: Back button pressed.");
	    	if(this.currentMenu){
	        	Logger.trace("[UserInterface - back]: Hiding current menu.");
	    		this.hideCurrentMenu();
	    		return;
	    	}
	    	if(this.dialogStack.length>0){
	        	Logger.trace("[UserInterface - back]: Hiding current dialog.");
	    		this.hideCurrentDialog();
	    		return;
	    	}
	    	var currentView = this.getCurrentViewControl();

	    	//We dont want to go back from right pane root view since the previous screen
	    	//is already showing in left pane
			if(this.combinedViews && currentView.id === this.splitPane.rightPaneRootView){
				//currentView.refresh();
				return;
			}

	    	if (currentView.back && !currentView.back.call(currentView.backContext?currentView.backContext:currentView, currentView)){
	        	Logger.trace("[UserInterface - back]: Called back callBack on view.");
	    		return;
	    	}
	    	this.hideCurrentView(array.some(currentView.eventHandlers, function(handler){
	        	Logger.trace("[UserInterface - back]: Hiding current view.");
			    ...
			    if (element.parentControl && element.parentControl._controlType) {
			    	...
			    	if (showControl._controlType == "View") {
			    		...
			    		if(this.getCurrentViewControl()){
			    			if(showControl.allowDrillDown == "true"){
				    			skipValidation = true;
				    		}
			    		}
				    	...
				    	if(typeof showControl.startView == 'undefined'){
				    		if(showControl.baseWidget.getShowingView() == showControl.baseWidget){
				    			...
				    			showControl.render();
				    			if (typeof showControl._resetDataBindings == "function") {
...
//				    	}
					    ...
					    Logger.trace('[UI - show] showing dialog: ' + id);
				    	this.dialogStack.push(element);
					    if(showControl.saveonshow == true){
						    Logger.trace('[UI - show] showing dialog: ' + id + ' saveonshow is true.');
					    	this.application.showBusy();
					    	ModelService.saveAll(this.application.getResourceArray()).then( function(){
							    Logger.trace('[UI - show] popping dialog ' + id + ' after save.');
		    ...
		    if (view) {
		    	...
		    	if(this.viewHistory.length==0 || this.viewHistory[this.viewHistory.length-1].id!=viewId){
		    		if(this.viewHistory.length>0 && viewId=="platform.LoginView") {
		    			...
		    			this.ensureCombinedViewHIstoryOrder(viewId);
		    		}
		    	}
		    }
	    },

	    //This function is needed because the view is pushed into stack after resources are available.
	    //This startegy does not work for splitpane because any of the two views in the panes can have 
	    //resources ready first and mess up the viewHistory stack
	    ensureCombinedViewHIstoryOrder: function(viewId){
	    	if(this.combinedViews && (viewId === this.splitPane.leftPaneRootView || viewId === this.splitPane.rightPaneRootView)){
	    		if(this.viewHistory[2] && this.viewHistory[3]){
		    		if(this.viewHistory[2].id === this.splitPane.rightPaneRootView && this.viewHistory[3].id === this.splitPane.leftPaneRootView){
		    			var temp = this.viewHistory[2];
		    			this.viewHistory[2] = this.viewHistory[3];
		    			this.viewHistory[3] = temp;
		    		}
	    		}
	    	}
	    }, 
	    ...
	    _hideDialog : function(id) {
		    ...
		    if (id) {
		    	Logger.trace("[UI._hideDialog] - Attempting to hide dialog: " + ((typeof id == 'string')? id : id.id));
			    var dialog = registry.byId(id); 
		    	if(!dialog || (dialog != this.dialogStack[this.dialogStack.length-1])){
		    		Logger.info("[UI._hideDialog] Can only hide top-most dialog");
		    		return;
		    	}
			    if (dialog) {
			    	Logger.trace('[UI - _hideDialog] hiding dialog: ' + id);
				    if(dialog.parentControl && dialog.parentControl.saveonhide == true){
				    	var self = this;
	    		    	Logger.trace('[UI._hideDialog] - Calling saveAll');
				    	ModelService.saveAll(this.application.getResourceArray()).
			    	    then( function(){
		    		    	...
		    		    	Logger.trace('[UI._hideDialog]  - Hiding dialog: ' + dialog.id);
			    	    	dialog.parentControl.hide();	
			    	    }).otherwise(function(error){
		    		    	Logger.traceJSON('[UI._hideDialog]  - Error return from saveAll: ', error);
			    	       self.application.hideBusy();
						    if(error && error.error){
						    	switch (true) {				
						    		case (error.error instanceof PlatformRuntimeException):										
		    	    	...
		    	    	while(this.dialogStack.top() && this.dialogStack.top().id == dialog.id) {
		    		    	Logger.trace("[UI._hideDialog] - Poping  dialog: " + this.dialogStack.top().id);
		    	    		this.dialogStack.pop();
		    	    	}
		    	    	...
		    	    	if (dialog.parentControl){
		    		    	...
		    		    	Logger.trace("[UI._hideDialog] - Unable to hide dialog: " + dialog.id + " because it done't have a parent control.");
		    	    	}
				    }
				    return;
			    }
		    	...
		    	Logger.trace('[UI._hideDialog] - unable to hide dialog because id is null');
		    }
	    },
	    ...
	    showExitMessage : function() {
	    	var except = new PlatformRuntimeException('showExitMessage called');
	    	Logger.trace('[UserInterface - showExitMessage] call stack:\n ' +  except.stack);
	    	return this.show('Platform.ExitApplicationPrompt');
	    },
	    
	    hideCurrentViewAfterEsig : function(cleanup) {
	    	Logger.trace('[UserInterface - hideCurrentViewAfterEsig] hiding current view.  cleanup: ' +  cleanup);
	    	this.movingBack = true;
	    	...
	    	if(this.combinedViews && 
	    			this.splitPane.leftPaneRootView === this.viewHistory[this.viewHistory.length - 2].id &&
	    			this.splitPane.rightPaneRootView === this.viewHistory[this.viewHistory.length - 1].id ){
	    		
	    		//If the current showing vies are root views of the split pane. Dont hide anything.
	    		return;
	    ...
	    returnToView : function(viewId, settingCurrentRecord) {
	    	

	    	if(settingCurrentRecord && this.combinedViews){
	    		this.movingBack = false;
	    	}
	    	else{
	    		this.movingBack = true;
					    ...
					    if (event == 'datachange' && handler) {
					    	...
					    	context.addHandler(on(bindElement, event, lang.hitch(context, function(e) {
							    ...
							    if (e.target && e.type === 'tap') {
							        if(this.ui.transitioning) //Should not register a tap during transition
							    		return;
	    ...
	    showToastMessage : function(message, type,position) {
			var toasterId = new Date().getTime();
			domConstruct.destroy("toastDiv");
			var toastDiv = null;

			if (this.combinedViews) {
				toastDiv = domConstruct.create("div", { id: 'toastDiv' }, dijit.byId('combined_view').getParent().domNode, "first");
			}
			else {
				toastDiv = domConstruct.create("div", { id: 'toastDiv' }, this.getCurrentView().getParent().domNode, "first");
			}

			var info = new Toaster({
				id: toasterId,
				positionDirection: position?position:'br-up',
				duration: 5000
			});
	    ...
	    showSnackBarMessage : function(message, type) {
			var toasterId = new Date().getTime();
			domConstruct.destroy("snackBarDiv");

			var toastDiv = null;
			
			if (this.combinedViews) {
				toastDiv = domConstruct.create("div", { id: 'snackBarDiv' }, dijit.byId('combined_view').getParent().domNode, "first");
			}
			else {
				toastDiv = domConstruct.create("div", { id: 'snackBarDiv' }, this.getCurrentView().getParent().domNode, "first");
			}
	    ...
	    isaCombinedView: function(){
	    	return this.primaryViewID === 'combined_view' && (this.viewHistory.slice(-1).pop().id === this.ui.mainSplitter.panes[1].firstViewId);
	    	//return this.primaryViewID === 'combined_view';
	    },

	    isCurrentViewFullScreen: function(){
	    	return this.getCombinedView().children[0].panes[1].domNode.clientWidth == 0 || this.getCombinedView().children[0].panes[0].domNode.clientWidth == 0
	    	//return this.ui.getShowingView().viewControl.domNode.clientWidth;
	    },
