...
function(declare, ControlBase, ContentPane, DatePicker, TimePicker, ValuePickerSlot, Label, touch, dijitFocus, on, lang, array, query, domAttr, domClass, MessageService, Logger, registry, domConstruct, SystemProperties) {
...
/**@memberOf platform.ui.control.DateTimePicker */
				...
				startup: function(){
					...
					this._handlers = [
						...
						this.connect(this.inputNode, "onblur", lang.hitch(this, function(e){
							this._onChange(e);
						}))
					];
		...
		build : function() {
			...
			if(this.date){
				...
				var dateValues = new Array(date.getUTCFullYear(),date.getMonth()+1,date.getDate()); 

				if( SystemProperties.getProperty('si.datepicker.overridedefaultyear') && SystemProperties.getProperty('si.datepicker.overridedefaultyear') == "true") {
					
					this.dateWidget = this.createWidget(DatePicker, {
						id: this.getId() + '_dPicker',
					   'class': 'datePicker',
						values: dateValues,
						// APAR IJ08075 setting the year limit to 9999
						slotProps: [
							// using the default values from dojox/mobile/ValuePickerDatePicker.js
							{labelFrom:1970, labelTo:9999, style:{width:"87px"}},
							{style:{width:"72px"}},
							{style:{width:"72px"}}
						]
					});
