...
function(declare, ContainerControlBase, ContentPane, domConstruct, Stateful, getStateful, lang, array, on, Logger, LabelManager, ModelService,
	...
	var pushNotificationTransformer = null;
	return declare([ OrientationEventListenerMixin ], {
	    constructor : function(options) {	  
		    ...
		    this.appId = (options && options.id) ? options.id : null;
...
/**@memberOf platform.ui.control.Application */
	    ...
	    registerNotificationDataHandler: function(handler){
	    	if(!pushNotificationTransformer){
	    		
	    		pushNotificationTransformer = handler;
	    	}

	    },

	    getNotificationDataHandler: function(){
	    	return pushNotificationTransformer
	    },
	    ...
	    banPushNotification: function(){
	    	topic.publish(PlatformConstants.BAN_PUSH_NOTIFICATION);
	    },

	    allowPushNotification: function(){
	    	topic.publish(PlatformConstants.ALLOW_PUSH_NOTIFICATION);
	    },
	    ...
	    hideBusy : function() {
	    	...
	    	this.hideBusyCtr = 0;
	    	this.showBusyState = false;
	    	this.busyIndicator.ignoreTimeout = false;
	    	try{
	    ...
	    addResource : function(resource) {
	    	...
	    	if (this.resources[resource.resourceID] != resource){
		    	...
		    	// only re-do in-memory sort if it isn't sorted by JSONStore (metadata.orderBy)
		    	if(resource && 'function' === typeof resource.getMetadata && !resource.getMetadata().serverOnlyMode && !resource.getMetadata().orderBy){
	    ...
	    showMessage : function(message, callback, callbackArgs) {
	    ...
	    showMessageWithContinue : function(message, okCallback, continueCallback, okId, continueId) {
	    	var dialogId = new Date().getTime();
		    var dlg = new Dialog({
		    	id: dialogId,
		    	onOrientationChanged : function() {
		    ...
		    var okBtn = this.ui.createWidget(Button, {
		    	id: 'message_ok',
			    'innerHTML' : okId?MessageService.createStaticMessage(okId).getMessage():MessageService.createStaticMessage('okbutton').getMessage()
		    }, true);
		    
		    var continueBtn = this.ui.createWidget(Button, {
		    	id: 'message_continue',
			    'innerHTML' : continueId?MessageService.createStaticMessage(continueId).getMessage():MessageService.createStaticMessage('continuebutton').getMessage()
		    }, true);
