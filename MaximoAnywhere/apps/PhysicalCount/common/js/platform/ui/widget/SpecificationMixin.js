...
function(arrayUtil, StoreListMixin, declare, lang, Deferred, all, on, touch, ListItem, ApplicationHandlerBase, ModelService, MessageService, Logger, ComboBox, Text, SynonymDomain) {
	...
	return declare([StoreListMixin], {
		...
		CLAZZ: 'platform.ui.widget.SpecificationMixin',
		...
		onComplete: function(/*Array*/items){
			...
			if (items.length > 0) {
				...
				if (self.maxdomains == null) {
					...
					ModelService.all('maxdomain').then(function(domains) {
						self.maxdomains = domains; 
						self.generateList(items);
						self.control.baseWidget.domNode.style.display='inline';
						//self.control.postCreate(); 
					})
					.otherwise(function(error) {
						Logger.traceJSON('[SPECIFICATIONS] maxdomain initialization has failed!: ', error, self.CLAZZ);
					});
				}
				else {
					self.generateList(items);	
            ...
            if (!this.dynamicBuild || !this.modelData) {
                ...
                switch (self.type) {
                    ...
                    case 'duration':
                        ...
                        break;
                    default:
                    	var woSpec = self.modelData.getMetadata().name == 'workOrderSpecResource' ? self.modelData : null;
                    	if(woSpec) {
                    		var domainSet = self.application.getResource('maxdomain');
...
//						control: parentControl,
				    ...
				    // overwrite Text.showMyLookup
				    inputControl.showMyLookup = this._woSpecShowMyLookup;
				}
				else {
					// combobox
					var props = {
