...
/**@memberOf platform.ui.widget._StoreListMixin */
		...
		_afterPageChange : function(resource, newIndex){
			...
			try{
				if(this.control.ui.combinedViews && !this.control.ui.isCurrentViewFullScreen() && !this.control.viewControl.isLookup())
				{
					this.control.application.showBusy();
					this.control.refresh();
					this.control.ui.getViewFromId(this.control.ui.splitPane.rightPaneRootView).refresh();
        ...
        createOverflowItem: function(hasPrevious, hasNext) {
            ...
            if (hasPrevious) {
                ...
                this.control.addHandler(on(prevWidget, tap, lang.hitch(prevWidget, function(e){
                	this.listControl.control.ui.emitBlur();
                	this.listControl.control.ui.emitBlur();
	    ...
	    setCurrentRecord : function(listItem){
	    	if(this.control.ui.isaCombinedView()){
	    		ModelService.saveAll(listItem.control.application.getResourceArray());
	    	}
		    ...
		    try {
		    	...
		    	if(committed || !transitionView.editableView){
			    	if(listItem.control.ui.isViewOnStack(listItem.transitionInfo.moveTo)>0){
			    		listItem.control.ui.returnToView(listItem.transitionInfo.moveTo, true);
			    	}
			    	else {
