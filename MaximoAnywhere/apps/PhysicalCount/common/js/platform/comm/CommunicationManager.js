...
define("platform/comm/CommunicationManager", [
	...
	var logClass = "platform.comm.CommunicationManager";
...
/**@memberOf platform.comm.CommunicationManager */
		CLAZZ: 'platform.comm.CommunicationManager',
		...
		pullResourceData: function(resourceMetadata, queryBase, whereClause, pageSize, includeAllRelatedRecords, applyStablePagination, oslcQueryParameters){
			...
			Logger.trace('[COMM] queryUrl: ' + finalUrl, 2, this.logClass);
		...
		pullResourceDataWithComplexAttributes: function(resourceMetadata, queryBase, whereClause, complexAttributeList, pageSize){
			...
			if (!resourceMetadata){
				Logger.trace('[COMM] invalidMetadata', null, this.logClass);
				throw new PlatformRuntimeException('invalidMetadata');
			}
			...
			if(this.shouldCancelRoundTrip()){
				var deferred = new Deferred();
				Logger.trace('[COMM] timeout interval not exceeded', null, this.logClass);
				deferred.reject("timeout interval not exceeded");
				return deferred.promise;
			}
			
			pageSize = (!pageSize) ? 300 : pageSize;  //safeguard in case somebody forgot to set a pageSize in resource
			Logger.trace('[COMM] pageSize ' + pageSize, null, this.logClass);
			...
			var urlBase = resourceMetadata.getURLBase();
			Logger.trace('[COMM] urlBase ' + urlBase, null, this.logClass);
			...
			Logger.trace('[COMM] queryString ' + queryString, null, this.logClass);
			
			resourceUrl = queryString;
			
			Logger.trace('[COMM] resourceUrl: ' + resourceUrl, 2, this.logClass);
			...
			invocationData.timeout = SystemProperties.getConnectivityTimeoutForStablePagination();
			var adapterDataFetchDeferred = invokeAdapterQueryMethod(invocationData);
			adapterDataFetchDeferred.always(lang.hitch(this, function(){
				Logger.timerEnd('[COMM] Sending url to adapter (pullResourceDataWithComplexAttributes)', null, this.logClass);
			}));
			return adapterDataFetchDeferred;
		...
		pullSingleResourceData: function(resourceMetadata, resourceUrl, /*optional*/ arrayOfFieldNames){
			...
			if (lang.isArray(arrayOfFieldNames)){
				arrayUtil.forEach(arrayOfFieldNames, function(fieldName){
					fieldsLog = fieldsLog + fieldName + ", ";
					if (resourceMetadata.isFieldComplex(fieldName)){
						requestedComplexAttributes.push(resourceMetadata.getField(fieldName));
					}
				});
			}
			if (fieldsLog.length > 2) {
				fieldsLog = fieldsLog.substring(0, fieldsLog.length-2);
			}
			var oslcSelect = null;
			if(lang.isArray(arrayOfFieldNames) && arrayOfFieldNames.length > 0){
				oslcSelect = this._getOslcSelectFromMetadataFields(resourceMetadata.fields, arrayOfFieldNames);
			}else{
			...
			then(function(oslcResult) {
				Logger.trace("[ADAPTER INVOKE] Fields to query: " + fieldsLog, null, this.CLAZZ);
				//Logger.trace("[ADAPTER INVOKE] Total time: " + oslcResult.responseJSON.totalTime + "ms", null, this.CLAZZ);
		...
		pushResourceData: function(resourceMetadata, changeRecord, complexAttributesToAskInResponse) {
			...
			arrayUtil.forEach(complexAttributesToAskInResponse, function(complexField){
				...
				if(complexFieldInfo && complexFieldInfo.selectExpression){
					...
					Logger.trace("[SYNCHRO] Not found metadata field for " + complexField, null, this.CLAZZ);
				}				
			});	
			...
			if(complexAttributesToAskInResponse[0] && resourceMetadata.isComplexFieldAnAttachment(complexAttributesToAskInResponse[0])){
				...
				arrayUtil.forEach(attachToUpload, function(attach){
					...
					var transId = null; 
					if(attach['attachmentTransId']){
						transId = attach['attachmentTransId'];
					}
					
					delete attach['attachmentTransId'];
					...
					if(transId){
						uploadOptions.transactionid = transId;
					}
			...
			Logger.timerStart('[SYNCHRO] Pushing resource data (pushResourceData)');
			...
			var encodedKey = resourceUrl.substring(resourceUrl.lastIndexOf('/')+2); // skip the first char in the id '_
        	var decodedKey = null;
        	try {
        		decodedKey = atob(encodedKey);
        	}
        	catch(e) {}
			Logger.trace("[SYNCHRO] Push transaction: " + invocationData.transactionid + ' | ' + resourceMetadata['name'] + ' | ' + decodedKey + ' | ' + resourceUrl, null, this.CLAZZ);
			UserAuthenticationManager.invokeAdapterSecurely(invocationData).
			then(function(response) {
				Logger.trace("[SYNCHRO] Push transaction OK: " + invocationData.transactionid + ' | ' + resourceMetadata['name'] + ' | ' + decodedKey + ' | ' + resourceUrl, null, self.CLAZZ);
			...
			otherwise(function(response) {
				...
				Logger.timerEnd('[SYNCHRO] Pushing resource data (pushResourceData)');
			});
