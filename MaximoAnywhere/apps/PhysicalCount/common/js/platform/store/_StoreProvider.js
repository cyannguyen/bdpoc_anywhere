...
function(LocalStorageCollection, Logger, SHA, lang, Deferred, topic, PlatformConstants, MessageService, storeProviderSecurity){
...
/**@memberOf platform.store._StoreProvider */
		init: function(stores, options){
			...
			var stringifiedClonedOptions = JSON.stringify(options);
   			...
   			if (clonedOptions['username']){
   				...
   				clonedOptions['username'] = SHA(clonedOptions['username'], hexOutputFormat);
   			}
		...
		_changePassword: function(/*String*/ username, /*String*/ oldPassword, /*String*/ newPassword) {
			...
			var hexOutputFormat = 1;
			var username = SHA(username || "-no-user-", hexOutputFormat);
			...
			if(!previousLoggedUsersJson[username]) {
				...
				deferred.resolve("Current user have never logged in, no need to change password");
				return deferred.promise;
			}
			...
			if(isEncrypted) {
				...
				// change password
				var hashedNewPassword = SHA(newPassword || "-no-pass-", hexOutputFormat);
			...
			setTimeout(function(){
				...
				if ('password' in credentials && 
	   			...
	   			if (credentials['username']){
	   				var hexOutputFormat = 1;
	   				credentials = {
	   					username: SHA(credentials['username'], hexOutputFormat),
