...
function(declare, Deferred, arrayUtil, lang, all, CommunicationManager, JsonTranslationMixin,
	...
	var logClass = "platform.store._ServerDataDownloadManager";
...
/**@memberOf platform.store._ServerDataDownloadManager */
		...
		_serverFetchWihtOslcWhere: function(queryBase, pageSize, oslcWhereClause){
			Logger.trace('[COMM] _fetchWihtOslcWhere "' + this._metadata.name + '" resource data request.');
			Logger.trace('[COMM] Getting data from adapter if connectivity is available.');
			var whereClause = oslcWhereClause || this._metadata.whereClause;
			var promise = this._getDataFromServerIfPossibleAndStore(queryBase, whereClause, pageSize);
		...
		_downloadAllPagesOfResourceData: function(cancelIndicator, queryBase, progressCallback, applyStablePagination){
			...
			var demoMode = SystemProperties.getProperty('demoMode.enabled');
			
			var limitData = ((WL.Client.getEnvironment() == WL.Environment.PREVIEW || demoMode ) && (this._metadata.isSystem || this._metadata.additionalData) && !WL.application.debug);
			...
			if (demoMode && (this._metadata.isSystem || this._metadata.additionalData) && !WL.application.debug) {
				var demopagesize = (SystemProperties.getProperty('si.demopagesize')) ? SystemProperties.getProperty('si.demopagesize') : 200 ;
				if (pageSize > demopagesize) {
					pageSize = parseInt(demopagesize);
				}
			}
						...
						then(function(oslcResult){
							...
							if(justQuery){
								...
								then(function(){
									...
									var metrics = self._metadata.getResourceMetrics(queryBase);
									if (metrics){
										if (oslcResult["maxrowstamp"] != null || oslcResult["maxrowstamp"] != undefined ) {
		...
		_getDataWithComplexAttributesFromServerIfPossibleAndStore: function(queryBase, whereClause, complexAttributeList, pageSize){
			...
			then(function(isConnected){
				if(isConnected){
					...
					Logger.trace('[COMM] No connection available, check the network and try again.', null, self.logClass);
				}
			});
		},
		...
		_serverGetChildrenResourceData: function(parentModelData, complexAttributeNames){
			var rootModelData = parentModelData.getRootModelData();
			var resourceRef = {remoteid: rootModelData.getRemoteId()};	
