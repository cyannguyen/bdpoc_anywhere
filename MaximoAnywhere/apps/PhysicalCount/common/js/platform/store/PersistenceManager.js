...
function(thisModule, Deferred, all, lang, arrayUtil, declare, jsonQuery, PlatformRuntimeException, StoreProvider, CommunicationManager, 
		...
		PlatformConstants, Logger, topic, runOrRejectWithError, DataCleanupService, storeProviderSecurity, SHA, UserAuthenticationManager) {
	...
	var classBody = {
		...
		_initUserProfileStore: function(credentials, force) {
			...
			var username = SHA(credentials['username'] || "-no-user-", hexOutputFormat);
			var password = SHA(credentials['password'] || "-no-pass-", hexOutputFormat);
				...
				if(previousLoggedUsersJson[username]) {
					...
					if(isEncrypted) {
						...
						if(previousLoggedUsersJson[username] == password) {
							...
							deferred.reject(MessageService.createStaticMessage("Unable to authenticate user on both server and locally"));
						}
					}
				}
		...
		_initStore: function(storeName, searchFields, credentials, force){
			...
			.fail(function(err){
				...
				err['recoverPassword'] = credentials['authenticated'] && credentials['previousUser'] && storeProviderSecurity.isEncrypted() && err.msg == 'INVALID_KEY_ON_PROVISION';
				deferred.reject(err);
			});
		...
		_activateCollections: function(credentials, failIfNotExist, force){
			...
			then(
				...
				function(err) {
					...
					if(err.recoverPassword){
						exception['recoverPassword'] = err.recoverPassword;
					}
					throw exception;
				}
			);
		...
		removeCollection: function(metadata) {
			if (metadata.isInited == false) {
				var self = this;
				return self.initStore(metadata).
				then(function(){
					return self.removeCollection(metadata);
				});
			}
			var jsonStore = this._getStore(metadata.name);
			return this._asDojoPromise(jsonStore.removeCollection());
		},	
		...
		_notifyDownloadWorkListDataReceived: function(resourceName, data) {
			
			if (data.length == 0){
				return;
			}
			try {
				topic.publish(PlatformConstants.WORKLIST_DATA_RECEIVED + '/' + resourceName, resourceName, data);
			} catch (e) {
				Logger.log('Error publishing WORKLIST_DATA_RECEIVED for resource ' + resourceName + '.' + e);
			}
		},
