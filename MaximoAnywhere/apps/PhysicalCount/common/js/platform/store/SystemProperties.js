...
/**@memberOf platform.store.SystemProperties */
		...
		loadPlatformProperties: function() {
			var self = this;
			var fileName = 'worklight.properties'
			var deferred = new Deferred();
			var result = {};
			...
			if(!systemPropertiesMap || Object.keys(systemPropertiesMap).length==0){
				...
				function successCallbAck(fileContentAsString){
					var fileLines = fileContentAsString.split('\n');

					arrayUtil.forEach(fileLines, function(line){
						if(line.trim()[0] !== '#' && line !== ""){
							var pair = line.split('=');
							if(pair.length === 2)
					...
					for(var name in result){
						self.setProperty(name, result[name], false);
					}
					deferred.resolve();
				}

				function failureCallbAck(){
					arrayUtil.forEach(platformProps, function(prop){
						result[prop] = "";
					})
					deferred.resolve();
				}
				
				WL.Client.readLocalFile(fileName, successCallbAck,failureCallbAck);
