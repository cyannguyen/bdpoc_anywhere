...
/**@memberOf platform.handlers.AdditionalDataDialogHandler */
		...
		confirmADDownload: function(eventContext){
			...
			//Demo Mode changes
			var demoMode = SystemProperties.getProperty('si.demomode');
			SystemProperties.setProperty('demoMode.enabled',demoMode=="true",true);
			//End Demo Mode Changes
		...
		showHideConcelOption: function(eventContext) {
			//var demoStore = eventContext.application.getResource("PlatformDemoModeResource").getCurrentRecord();
			var demoMode = SystemProperties.getProperty('demoMode.enabled') == null ? false : SystemProperties.getProperty('demoMode.enabled') ;
			
			if (demoMode) {
				eventContext.setDisplay(false) ;
			} else {
				//First time login with Demo Mode no cancel allowed
				if (SystemProperties.getProperty('demoMode.enabled') == null) {
					eventContext.setDisplay(false) ;
				} else {
