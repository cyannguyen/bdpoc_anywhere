...
/**@memberOf platform.handlers.WorkOfflineHandler */
		...
		_processCallOuts: function(resourceName, queryBase){
			if (this.callBackStack.length > 0){
				...
				callBack(options).then(function(){
					self._processCallOuts(resourceName, queryBase);
					...
					if (!self.cancelTriggered){
						...
						if (self._allowContinue() && (error == "Unable to connect to server")){
							self._showDownloadFailedDialogWithContinue('downloadFailedNoConnectivity', lang.hitch(self,self._afterCancel),
									function(){
										//User wants to continue so add back handler and try again
										self.callBackStack.unshift(callBack);
										self._processCallOuts(resourceName, queryBase);
									}
							); 
						}
		...
		_showDownloadCompleteDialog: function(progressInfo, additionalMessage){
			
			var recordsDownloaded = progressInfo.recordsDownloaded['downloaded'];
			recordsDownloaded = (recordsDownloaded || 0);
			var attachmentSummary = (progressInfo.attachmentProgress || 0);
			...
			if(this.application.ui.combinedViews){
				message += '<br/><br/>' + MessageService.createStaticMessage("downloadRefreshList").getMessage();
				var viewControl = self.ui.getViewFromId(self.ui.viewHistory[self.ui.viewHistory.length - 2].id)
		
				var okCallback = function(){
					viewControl.lists[0].splitPaneModeSaveAndReload();
				}

				var continueCallback = function(){
					viewControl.lists[0].showRefreshButton();
				}
				this.application.showMessageWithContinue(message, okCallback, continueCallback, 'Refresh');
			}else{
				this.application.showMessage(message, function() {
					var viewControl = null;
					
					viewControl = self.application.ui.getCurrentViewControl();
					
					if (viewControl && viewControl.lists && viewControl.lists[0] && viewControl.lists[0].refreshButton){
...
//				self.refreshWorkList(eventContext);
		...
		doClearChanges: function(eventContext) {
			...
			if (this.savedEventContext._controlType == 'Link') {
				...
				then(function(){
					try{
						//Only hide the detail panel if we aren't running Split Pane
