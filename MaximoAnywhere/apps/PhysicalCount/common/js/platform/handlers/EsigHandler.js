...
/**@memberOf platform.handlers.EsigHandler */
		initializeEsig: function(eventContext){
			...
			if (username) {
				esigForm.set(authkey1, username);
			}
			else{
				esigForm.set(authkey1, null);
			}
			esigForm.set(authkey2, null);
...
//			esigAttribute = resourceAttribute;
		...
		_logEsigAttempt: function(eventContext, esigForm){
			...
			if (this.esigResource){
				...
				if (identifier && identifier.name){
					if(!this.esigResource.getCurrentRecord){
						recordId = this.esigResource[identifier.name];
					} else {
