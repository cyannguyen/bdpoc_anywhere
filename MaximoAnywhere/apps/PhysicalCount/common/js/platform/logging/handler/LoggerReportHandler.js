...
function (declare, ApplicationHandlerBase, ModelService, Memory, cookie, Logger, SystemProperties, numberUtil, TransactionLogger, PersistenceManager, MessageService, arrayUtil, lang, HtmlEntities, CommunicationManager) {
    ...
    return declare(ApplicationHandlerBase, {
        ...
        _creatTextEmail: function (networkInfo, eventContext, sendTransLog) {
            ...
            require(["platform/translation/MessageService","platform/auth/UserAuthenticationManager"],
					function (MessageService, UserAuthenticationManager) {
						...
						if (networkInfo.telephonyNetworkType) {
             ...
             WL.Device.getNetworkInfo(function (networkInfo) {
                require(["platform/auth/UserAuthenticationManager"],
						function (UserAuthenticationManager) {
						    ...
						    if (WL.Client.getEnvironment() == WL.Environment.WINDOWS8) {
						        inner += '<div><p style="text-indent: 3em;">';
						        inner += MessageService.createStaticMessage('LogLocation').getMessage(this);
						        inner += '</p><p style="text-indent: 3em;">';

						        inner += Windows.Storage.ApplicationData.current.localFolder.path;
        ...
        showIfConnected: function(eventContext) {
        	CommunicationManager.checkConnectivityAvailable()
        	.always(function(hasConnectivity){
        		eventContext.setDisplay(hasConnectivity);
        	});
        },
        
        uploadCurrent: function( eventContext ) {
        	console.log('uploading log');
        	var loggerStore = eventContext.logger.loggerStore;
        	...
        	window.resolveLocalFileSystemURL(filePath, function(dir) {
        		...
        		dir.getFile(fileName, {create:true}, function(file) {
        			...
        			fileEntry.createWriter(function(writer) {            			
            			var data = loggerStore.data;
            			var logText = '';
            			for ( var i = 0 ; i < data.length ; i++ ) {
            				logText = logText + data[i].msg.toString() + "\r\n";		
            			}
            			let text = new Blob([ logText ], { type: "text/plain" });
            			writer.write( text );
            			
            		}, function (error) { console.log ( error ) });  
					console.log( fileName );
            		console.log( filePath );
            		eventContext.ui.showMessage( fileName + ":  " + filePath); 
