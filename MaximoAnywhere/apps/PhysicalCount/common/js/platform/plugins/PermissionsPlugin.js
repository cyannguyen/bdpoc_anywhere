...
define("platform/plugins/PermissionsPlugin", [
    ...
    return {
        ...
        showPermissionsError: function() {
        	...
        	var excpt = new PlatformRuntimeException('errorGrantingPermissions');
        	if (typeof this.permissionsErrorCallback == 'function'){
        		this.permissionsErrorCallback(excpt);
        		this.permissionsErrorCallback = null;
        	}
        	else{
        		throw excpt;
        	}
        },
        permissionsErrorCallback: null,
        checkAndGrantPermissions: function(permissionToCheck, permissionsToGrant, callbackobj, callbackfunctionname, callbackfunctionargs, onPermissionsError) {
			...
			if (WL && WL.Client && WL.Client.getEnvironment()==WL.Environment.ANDROID) {
				if (onPermissionsError){
					this.permissionsErrorCallback = onPermissionsError;
				}
