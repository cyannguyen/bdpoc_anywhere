...
/**@memberOf platform.map.MapProperties */
		...
		getProperties: function(){
			...
			esriMapProps = [
			   ...
			   'si.map.esri.useMobileFirstProxy',
			   'si.map.offline.pageSize',
			   'si.map.offline.queryFeaturesDelay',
			   'si.map.offline.featuresLimit',
			   'pluss.si.map.esri.measureLengthUnit',
			   'pluss.si.map.esri.measureAreaUnit',
               'androidLocalMapUrl',
               'windowsLocalMapUrl',
               'iosLocalMapUrl',
               'provider',
			...
			for(var i=0; i<esriMapProps.length; i++){
				...
				switch(mapPropName){
					...
					case "si.map.openLayers.maxZoom": 
						...
						break;
					case "si.map.esri.useMobileFirstProxy":
						mapProp = {mapPropName: mapPropName, propertyValue: this.getUseMobileFirstProxy()};
						break;
					case "si.map.offline.pageSize":
						mapProp = {mapPropName: mapPropName, propertyValue: this.getOfflinePageSize()};
						break;
					case "si.map.offline.featuresLimit":
						mapProp = {mapPropName: mapPropName, propertyValue: this.getOfflineFeaturesLimit()};
						break;
					case "si.map.offline.queryFeaturesDelay":
						mapProp = {mapPropName: mapPropName, propertyValue: this.getOfflineQueryTimeDelay()};
						break;
					default:
		...
		getOfflinePageSize: function() {
			var pageSize = this.getProperty('si.map.offline.pageSize');
			try{
				if(pageSize && Number(pageSize)){
					return pageSize;
				}
				Logger.trace(this._className + " - Value: "+this.getProperty('si.map.offline.pageSize')+" is an invalid value for property 'si.map.offline.pageSize'");
			}
			catch(error){
				Logger.trace(this._className + " error on property 'si.map.offline.pageSize' : "+error);
			}
			
			return 20000;
		},
		
		getOfflineFeaturesLimit: function() {
			var featuresLimit = this.getProperty('si.map.offline.featuresLimit');
			try{
				if(featuresLimit && Number(featuresLimit)){
					return featuresLimit;
				}
				Logger.trace(this._className + " - Value: "+this.getProperty('si.map.offline.featuresLimit')+" is an invalid value for property 'si.map.offline.featuresLimit'");
			}
			catch(error){
				Logger.trace(this._className + " error on property 'si.map.offline.featuresLimit' : "+error);
			}
			
			return 100000;
		},
		
		getOfflineQueryTimeDelay: function() {
			var queryTimeDelay = this.getProperty('si.map.offline.queryFeaturesDelay');
			try{
				if(queryTimeDelay && Number(queryTimeDelay)){
					return queryTimeDelay;
				}
				Logger.trace(this._className + " - Value: "+this.getProperty('si.map.offline.queryFeaturesDelay')+" is an invalid value for property 'si.map.offline.queryFeaturesDelay'");
			}
			catch(error){
				Logger.trace(this._className + " error on property 'si.map.offline.queryFeaturesDelay' : "+error);
			}
			
			return 1500;
		},
		
		getUseMobileFirstProxy: function() {
			return "0";
		},
