...
function(declare, on, AbstractMap, Deferred, lang, Logger, RouteRequest, DirectionsRenderer, 
	...
	EventDispatcher, MessageService, JSEvent, domGeom, MapProperties, tpkmanager, featureServiceManager,
	...
	return declare(AbstractMap, {
		...
		ANYWHERE_PROVIDER_MAPMANAGER : "mapmanager",
		MAPMANAGER_PROVIDER_SPATIAL : "spatial",
		MAPMANAGER_PROVIDER_BING : "bing",
		...
		constructor: function(/*String*/ providerUrl, /*JSONObject*/ container, /*JSONObject*/ specificParameters) {
			...
			this.mobileMaximoSpatial = spatialMapHandler['mobileMaximoSpatial' + viewId];
			if (this.mobileMaximoSpatial == null) {
				this.mobileMaximoSpatial = spatialMapHandler['mobileMaximoSpatial'];
				if (this.mobileMaximoSpatial == null) {
		...
		init : function () {
			...
			if (provider && provider == MapProperties.ANYWHERE_PROVIDER_MAPMANAGER) {
				var mapManagerPromise = this._loadMapManager();
				
				mapManagerPromise.then(lang.hitch(this, function () {
					var promiseAfterInit = this.afterInit();
					promiseAfterInit.then(lang.hitch(this, function () {
						initPromise.resolve();
					})).otherwise(function () {
						initPromise.reject();
					});
				})).otherwise(lang.hitch(this,function (errorInformation) {
					WL.application.hideBusy();
					var errorMessage = MessageService.createResolvedMessage(errorInformation[0], 
							[errorInformation[1]]);
...
/**@memberOf platform.map.OpenLayersMap */
							...
							if (!this._map) {
								switch(provider.toLowerCase()) {
									case "osm":
										...
										break;
									case "bing":
										this._instantiateBingMap(target, zoom, centerx, centery);
										break;
									case "google":
										Logger.warn(MessageService.createStaticMessage('noSupportForGoogleMaps').getMessage());
										break;
									case "esri":
										var that = this;
									    ...
									    var xyzproviderurl = (providerUrl) ? providerUrl : "http://services.arcgisonline.com/ArcGIS/rest/services/World_Street_Map/MapServer/tile/{z}/{y}/{x}"; 
									    // url can't be blank for offline windows map to work
									...
									case MapProperties.ANYWHERE_PROVIDER_MAPMANAGER:
										
										resolveDeferred = false;
										
										if (this.mapManager) {
											
											switch (this.mapManager.mapprovider.toLowerCase()) {
												case MapProperties.MAPMANAGER_PROVIDER_BING:
													zoom = this.mapManager.currentMapSite["spi:zoomlevel"];
													centerx = this.mapManager.currentMapSite["spi:initialx"];
													centery = this.mapManager.currentMapSite["spi:initialy"];	
													
													if (this.mapManager.bmapslicense) {
														this._instantiateBingMap(target, zoom, centerx, centery, 
																this.mapManager.bmapslicense);
														
														this._configureMapMarkers();
														deferred.resolve();
													} else {
														WL.application.hideBusy();
														var bingMapsKeyMissingMessage = MessageService.createStaticMessage('bingMapsKeyMissing').getMessage();
														Logger.log(bingMapsKeyMissingMessage);
														var errorMessage = MessageService.createResolvedMessage('bingMapsKeyMissing');
														this._showErrorMsg(errorMessage);														
													}
													
													break;
													
												case MapProperties.MAPMANAGER_PROVIDER_GOOGLE:
													WL.application.hideBusy();
													var noSupportForGoogleMessage = MessageService.createStaticMessage('noSupportForGoogleMaps').getMessage();
													Logger.log(noSupportForGoogleMessage);
													var errorMessage = MessageService.createResolvedMessage('noSupportForGoogleMaps');
													this._showErrorMsg(errorMessage);
													
													break;
													
												case MapProperties.MAPMANAGER_PROVIDER_SPATIAL:
													//ModelService and map logic
													Logger.trace("Spatial Maps Logic Starts Here");		
													var spatialMapHandler = WL.application["platform.handlers.spatial.SpatialMapHandler"];
													mobileMaximoSpatialCurrentView = spatialMapHandler['mobileMaximoSpatial' + this.getLastMapViewId()];
													if (mobileMaximoSpatialCurrentView == null) {
														this.mobileMaximoSpatial.mapLoaded = false;
													}
													
													if (this.mobileMaximoSpatial.mapLoaded != true) {
														this.mobileMaximoSpatial.mapLoaded = false;
															
													this.mobileMaximoSpatial.createMap( target, lang.hitch(this, function(mapInstance) {
															
															this.mobileMaximoSpatial.mapLoaded = true;
															
															//Callback function
															var spatialMapHandler = WL.application["platform.handlers.spatial.SpatialMapHandler"];
															var viewId = this.getLastMapViewId();
															spatialMapHandler['mobileMaximoSpatial' + viewId] = this.mobileMaximoSpatial;
															spatialMapHandler.mobileMaximoSpatial = this.mobileMaximoSpatial;
		...
		_checkNetworkConnection: function() {
			...
			var spatialInstalled = (this.mapType == "mapmanager" && this.mapManager.mapprovider.toLowerCase() == "spatial")
			if(providerUrl && !spatialInstalled){
		...
		*/
		...
		showMap: function() {
    		...
    		if (provider == "mapmanager") {
    			var viewId = this.getLastMapViewId();
    			var spatialMapHandler = WL.application["platform.handlers.spatial.SpatialMapHandler"];
	    		var mobileMaximoSpatial = spatialMapHandler['mobileMaximoSpatial' + viewId];
				if (mobileMaximoSpatial == null) {
					mobileMaximoSpatial = spatialMapHandler['mobileMaximoSpatial'];
				}
				if (mobileMaximoSpatial != null) {
					this._map = mobileMaximoSpatial.map;
				}
    		}
		...
		hideMap: function() {
			...
			//If provider is SPATIAL is necessary to disable click events
			if (this.mapType == "mapmanager" && this.mapManager.mapprovider.toLowerCase() == "spatial"){
				var spatialMapHandler = WL.application["platform.handlers.spatial.SpatialMapHandler"];
				this.mobileMaximoSpatial = spatialMapHandler["getMobileMaximoSpatialInstance"]();
		...
		addLayer: function(/*_ModelDataSet*/ resource, /*String*/ layerId, /*int*/ resourceIndex) {
			...
			// Remove the markers layers to not duplicate the markers on the map.
        	if (this._map) {
    			...
    			if (layersArray != null) {
    				array.forEach( layersArray , lang.hitch( this, function ( layer, i ){
    					if(layer.get("name") == this.MARKERS_LAYER_NAME) {
    						layerToRemove = layer;
    					}
    				}));
		...
		removeAllLayers: function(checkTokenMXSpatial) {
			...
			if (this.mapType == "mapmanager" && this.mapManager.mapprovider.toLowerCase() == "spatial"){
				...
				if (viewId != null) {
					...
					this.mobileMaximoSpatial = spatialMapHandler['mobileMaximoSpatial' + viewId];
					spatialMapHandler.mobileMaximoSpatial = this.mobileMaximoSpatial;
					Logger.trace("Map View Id used " + viewId);
				}
				
				if (this.mobileMaximoSpatial) {
					var promiseUpdateLayers = this.mobileMaximoSpatial.updateLayersToolChanges();
		...
		_adjustExtentView: function(){
			...
			//If Provider is spatial we do not change the zoom or extent when a menu is opened
			if (this.mobileMaximoSpatial && this.mapManager &&
					this.mapManager.mapprovider.toLowerCase() == MapProperties.MAPMANAGER_PROVIDER_SPATIAL) {
				var spatialZoomStatus = this.mobileMaximoSpatial.zoomStatus;
				if (spatialZoomStatus) {
					this.zoomStatus = spatialZoomStatus;
				}
		...
		_switchMarker: function(/*ol.Feature*/ feature, /*boolean*/ selected) {
			if (feature) {
				...
				var iconJson = {
					...
					opacity: 1,
					src: null,
					imgSize: [markerModel.width, markerModel.height]
				}
		...
		/*
		 * Openlayer fix for getResolutionForExtent
		 */
		...
		_loadMapManager : function () {
			var deferred = new Deferred();
			
			var currentUserSite = this.userAuthenticationManager.currentUserSite;
			var mapManagerMeta = ResourceMetaData.getResourceMetadata( "mapmanager" );
			mapManagerMeta.setWhereClause( "spi:ismobile=1 and spi:active=1" );
			
			var mapManagerData = ModelService.all( 'mapmanager', null, null, true );
			mapManagerData.then( lang.hitch( this, function ( mapManagerSet ) {
				...
				if ( this.mapManager == null ) {
					deferred.reject(['noMapManagerFound', currentUserSite]);
				} else {
		...
		_instantiateBingMap : function(target, zoom, centerx, centery, apiKey) {
			var key = apiKey;
			var imagerySet = 'Road';
