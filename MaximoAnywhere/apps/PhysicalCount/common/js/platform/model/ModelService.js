...
/**@memberOf platform.model.ModelService */
		...
		/**
		 * Fetch data from the server using the passed in oslcWhere, and stores in the the local store.  Then returns data from the local storage using the passed
		 * in localquery.  Use this in place of filtered when you need to query the server with an oslc where that cannot be used
		 * to fetch data in local storage.  
		 * different than 
		 * @param resourceName <Required>- Name of resource
		 * @param oslcWhere <Optional>- Must be url encoded and us oslc named attributes.  Example: spi:location{oslc:shortTitle in [%22BR400%22,%22BR450%22 ]} and spi:siteid=%22BEDFORD%22
		 * @param localquery <Optional>- Query object used to fetch data from local storage
		 * @param queryBase <Optional> -  Name of querybase to use
		 * @param pageSize <Optional> - The number of record to fetch.  Defaulted to resource defined page size
		 * @returns ModelDataSet. 
		 */
		allWIthOslcWhereAndLocalQuery: function(resourceName, oslcWhere, localquery, queryBase, pageSize, isExactMatch){
			//used array.map instead of lang.clone as we just need a shallow copy	
			if(!localquery && !oslcWhere){
				return this.all(resourceName, queryBase, pageSize);
			}
			var metadata = this._getResourceMetadataOrThrowError(resourceName);
			if (metadata.isLocal()) {
				return this.filtered(resourceName, queryBase, localquery, pageSize, null, isExactMatch);
			}
			if ((metadata.isInited == false)) {
				var self = this;
				return PersistenceManager.initStore(metadata).
				then(function() {
					return self.allWIthOslcWhereAndLocalQuery(resourceName, oslcWhere, localquery, queryBase, pageSize, isExactMatch);
				});
			}
			pageSize = pageSize || metadata.pageSize;
			localquery = convertExactMatchAttributesInFilter(metadata, localquery);

			var dataAccessStrategy = new ServerPreferredDataAccessStrategy(metadata);
			var promise = dataAccessStrategy.fetchFromServerThenLocalQuery(queryBase, pageSize, oslcWhere, localquery, isExactMatch);
			return this._getDeferredResultFromPromise(promise, metadata, queryBase);
		},
		...
		**/
		...
		reloadAllPages: function(resourceName, queryBase){
			...
			var metrics = metaData.getResourceMetrics(queryBase);
			if (!metrics){
				return this.all(resourceName, queryBase); //Need metrics to download more than one page
			}
			var deferred = new Deferred();
			var self = this;
