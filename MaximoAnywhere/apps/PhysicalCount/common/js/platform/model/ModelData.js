...
/**@memberOf platform.model.ModelData */
		...
		evaluateFormulaFields: function(){
			FormulaEvaluator.evaluateFormula(this.getMetadata(), this);
		},
		isModified: function(){
...
//				to prevent holding multiple references to ModelDataSet
			...
			arrayUtil.forEach(attributeNames, function(attributeName) {
				...
				if (field){
					...
					if (field.isComplex && 
						...
						if (fieldValue && fieldValue != PlatformConstants.EMPTY_COMPLEX_FIELD){
		...
		getRootModelData: function() {
			var parent = this.getOwner().getParent();
			if(parent) {
				return parent.getRootModelData();
			}
			else {
				return this;
			}
