...
/**@memberOf platform.ui.control._BoundControlMixin */
				    ...
				    if (attributeInfo !== null) {
					    ...
					    return at(modelData, attributeName).transform({
					        format : function(value) {
					        	...
					        	//Classification number attributes require more two digit precision
					        	...
					        	if (!(isNumericField && dataType === 'integer')) {
					        	...
					        	//Only want to encode vaule if it's not being places in and active textfield or textarea.  If encoded then the input will show the encoding
					        	//Also only need to encode field that are are datatype string and usage not a number, boolean, or date/time
		...
		//#region Loc-In: Customize format dateTime 
		...
		formatDisplayedValue: function (value) { //allows additional display formatting for non-editable values
			...
			if (modelData !== null) {
				var metadata = modelData.getMetadata();
				if (metadata !== null) {
					var attributeInfo = metadata.getField(this.resourceAttribute); if (attributeInfo) {
						if (this.editable == false) {
							if (attributeInfo.dataType == 'datetime') {
								return value.replace('    ', '&nbsp;&nbsp;&nbsp;&nbsp;');
							}
							...
							if (value) {
								if (attributeInfo.dataType == 'datetime') {
									//return dojo.date.locale.format(new Date(value), { datePattern: 'dd/MMM/yyyy hh:mm a', selector: 'date' })
									return dojo.date.locale.format(new Date(value), { datePattern: 'MMM/dd/yyyy hh:mm a', selector: 'date' })
								} else if (attributeInfo.dataType == 'date') {
									//return dojo.date.locale.format(new Date(value), { datePattern: 'dd/MMM/yyyy', selector: 'date' })
									return dojo.date.locale.format(new Date(value), { datePattern: 'MMM/dd/yyyy', selector: 'date' })
								} else if (attributeInfo.dataType == 'time') {
									return dojo.date.locale.format(new Date(value), { timePattern: 'hh:mm', selector: 'time' })
								}
							} else {
								return value;
							}
		...
		//#endregion Loc-Out: Customize format dateTime 
