...
define("platform/ui/control/View", [
    ...
    "platform/ui/widget/NotificationModal",
    "platform/logging/Logger",
    "platform/handlers/WorkOfflineHandler",
    "platform/util/XSSSanitizer",
], function (
    //TODO: WorkOfflineHandler here is a workaround just for be able to use ios 12, need fix it properly.
    declare,
    Actions,
    ...
    var push_icon_display = false;
    var push_modal_display = false;
    test_cntr = 0;

    /**@class platform.ui.control.View */
    return declare([ContainerControlBase, OrientationEventListenerMixin, WorkOfflineHandler], {
        //TODO: WorkOfflineHandler here is a workaround just for be able to use ios 12, need fix it properly.
        id: null,
        label: "",
        baseWidget: null,
        ...
        showKeyboard: false,
        showOverflow: true,
        overflowImage: "header_overflow_OFF.svg",
        overflowMenu: "platform.overflow",
        querySelectorMenu: "platform.querySelector",
        showCancelButton: false,
        cancelButtonImage: "header_split_cancel_OFF.svg",
        transition: "slide",
        ...
        sync: function (eventContext) {
            if (
                this &&
                this.application &&
                this.application["platform.handlers.WorkOfflineHandler"]
            ) {
                var workOfflineHandlerObject =
                    this.application["platform.handlers.WorkOfflineHandler"];
                if (workOfflineHandlerObject) {
                    workOfflineHandlerObject.sync(eventContext);
                    console.log("View.js: calling sync from platform.handlers.WorkOfflineHandler");
                    this.logger.trace(
                        "View.js: calling sync from platform.handlers.WorkOfflineHandler"
                    );
                }
            }
        },
        ...
        build: function () {
            ...
            if (acessible && this.scrollable) this.build_accessible_elements = true;
            ...
            if (WL.Client.getEnvironment() == WL.Environment.PREVIEW) {
                this.baseWidget.slideTo = function (
                    /*Object*/ to,
                    /*Number*/ duration,
                    /*String*/ easing
                ) {
            ...
            var isCombinedViewChildrenTransitioning = function (thisObj) {
                return (
                    (thisObj.id === thisObj.ui.splitPane.leftPaneRootView &&
                        thisObj.ui.getCurrentView().id ===
                            thisObj.ui.splitPane.rightPaneRootView) ||
                    (thisObj.id === thisObj.ui.splitPane.rightPaneRootView &&
                        thisObj.ui.getCurrentView().id === thisObj.ui.splitPane.leftPaneRootView)
                );
            };

            var afterTransitionFunc = function (lt) {
                //In combined view after transition out is called before BeforTransitionIn
                //and hence we nned to put proper checks since getCurrentView is a different
                //instance in this case
                if (lt.ui.combinedViews) {
                    if (lt.ui.getViewFromId(lt.ui.transitionInfo.id).viewControl.isLookup()) return;
                } else {
                    if (lt.ui.getCurrentViewControl().isLookup()) {
                        return;
                    }
                }
                if (lt.ui.movingBack) {
                    lt._removeResourceWatches();
                }

                lt.destroyChildren();
                lt.transitioningOut = false;

                var currView = lt.ui.getCurrentView();
                var self = lt;
                if (
                    lt.application.isFeatureEnabled("pushnotification.enabled") &&
                    currView.movedFrom === "platform.LoginView" &&
                    currView.id === lt.ui._getPrimaryViewID()
                ) {
                    ModelService.all("PlatformTempPushNotification", null).then(function (
                        recordSet
                    ) {
                        recordSet.resourceID = "PlatformTempPushNotification";
                        self.application.addResource(recordSet);
                        if (recordSet.count() !== 0) {
                            self.showNotificationIcon(recordSet);
                            self.showNotificationModal(recordSet);
                        }
                    });
                }
            };
            ...
            on(
                this.baseWidget,
                "AfterTransitionOut",
                lang.hitch(this, function (moveTo, dir, transition, context, method) {
                    this.transitionInOutFLag = true;
                    afterTransitionFunc(this);
                })
            );
                            ...
                            lang.hitch(this, function () {
                                if (window.innerHeight + 100 < window.outerHeight) {
                                    ...
                                    if (curNode) {
                                        window.scrollTo(0, 0);
                                        this.ui
                                            .getCurrentViewControl()
                                            .scrollDomNodeIntoView(curNode);
                                    }
                                }
                            }),
                            500
            ...
            on(
                ...
                lang.hitch(this, function (moveTo, dir, transition, context, method) {
                    ...
                    //Calling this before rather than at the function end.
                    if (
                        !this.transitionInOutFLag &&
                        this.ui.combinedViews &&
                        this.id !== "platform.LoginView"
                    ) {
                        //We do a check if the left and right pane are first
                        //views transitioinng because we should not destro left pane for right pane view transition
                        if (
                            this.ui.getShowingView().id !== "combined_view" &&
                            !isCombinedViewChildrenTransitioning(this)
                        ) {
                            afterTransitionFunc(this.ui.getViewFromId(this.ui.getCurrentView().id));
                        }
                    }
                    ...
                    //TODO: WorkOfflineHandler and sync here is a workaround just for be able to use ios 12, need fix it properly.
                    //this.sync(context);
                    if (
                        WL.Client.getEnvironment() == WL.Environment.IPHONE ||
                        WL.Client.getEnvironment() == WL.Environment.IPAD
                    ) {
                        if (!this._isConnected && this._isConnected != null) {
                            localStorage.setItem("wasOffline", "true");
                        }

                        var wasOffline =
                            localStorage.getItem("wasOffline") &&
                            (localStorage.getItem("wasOffline") == "true" ||
                                localStorage.getItem("wasOffline") == "false")
                                ? localStorage.getItem("wasOffline")
                                : "false";
                        if (this._isConnected && wasOffline === "true") {
                    ...
                    if (resource && typeof this.labelWatchHandle == "undefined") {
                        this.labelWatchHandle = resource.onChange(
                    ...
                    var appName =
                        MessageService.createStaticMessage("applicationName").getMessage();
                    if (appName && SystemProperties.getProperty("demoMode.enabled")) {
                        this.ui.showSnackBarMessage(
                            MessageService.createResolvedMessage("DemoModeIndicator", [appName])
                        );
                    }

                    if (
                        this.application.isFeatureEnabled("pushnotification.enabled") &&
                        this.id !== "platform.LoginView" &&
                        this.id !== PlatformConstants.EMPTY_VIEW
                    ) {
                        var dataSet = this.application.getResource("PlatformTempPushNotification");
                        if (push_icon_display) {
                            this.showNotificationIcon(dataSet);
                        } else {
                            this.hideNotificationIcon();
                        }
                        if (push_modal_display) {
                            this.showNotificationModal(dataSet);
                        } else {
                            this.hideNotificationModal();
                        }
                    }

                    this.transitionInOutFLag = false;
                })
            );
            ...
            on(
                ...
                lang.hitch(this, function (moveTo, dir, transition, context, method) {
                    ...
                    //In the current situation, the loading message is hidden anyways by List, without waiting for lookup
                    //Update: we kill the loading even if it is lookup,
                    //since in the current scenario the loading message is hidden anyways by the list build
                    ...
                    if (acessible)
                        this.scrollWidget.slideTo(
                            { y: this.mainViewFooter.domNode.clientHeight },
                            0.3,
                            "ease"
                        );
                    this.baseWidget = this.mainView;
                    ...
                    var self = this;
                    var currView = this.ui.getCurrentView();
                    if (
                        this.application.isFeatureEnabled("pushnotification.enabled") &&
                        currView.movedFrom === "platform.LoginView" &&
                        currView.id === this.ui._getPrimaryViewID()
                    ) {
                        ModelService.all("PlatformTempPushNotification", null).then(function (
                            recordSet
                        ) {
                    ...
                    if (this.ui.combinedViews && this.id === this.ui.splitPane.leftPaneRootView) {
                        var idx = this.lists[0].listWidget.startIndex > 0 ? 1 : 0; //is it page 2 or first page?
                        var listChildren = this.lists[0].listWidget.getChildren();

                        if (listChildren && listChildren.length > 0) {
                            this.ui.removeOverlay(
                                this.ui.getViewFromId(this.ui.splitPane.rightPaneRootView)
                                    .baseWidget
                            );
                            var listItem = this.lists[0].listWidget.getChildren()[idx];
                            this.lists[0].listWidget.setCurrentRecord(listItem);
                        } else {
                            //There are no rows in the left pane
                            this.ui.addOverlay(
                                this.ui.getViewFromId(this.ui.splitPane.rightPaneRootView)
                                    .baseWidget
                            );
        ...
        hideSoftKeyBoard: function () {
            if (WL.Client.getEnvironment() == WL.Environment.ANDROID) {
                ...
                cordova.exec(
                    function (object) {
                        self.logger.trace(
                            "[DeviceKeyBoardPlugin] resolving with object: " + object
                        );
                    },
                    function (message) {
                        self.logger.error(
                            "[DeviceKeyBoardPlugin] failed to resolve, error message: " + message
                        );
                    },
                ...
                if (dijitFocus.curNode) dijitFocus.curNode.blur();
                setTimeout(() => {
                    domStyle.set(
                        dom.byId(WL.application.ui.transitionInfo.id),
                        "height",
            ...
            if (firstEditable) {
                ...
                if (WL.Client.getEnvironment() == WL.Environment.ANDROID) {
                    ...
                    cordova.exec(
                        ...
                        function (message) {
                            self.logger.log(
                                "[DeviceKeyBoardPlugin] failed to resolve, error message: " +
                                    message
                            );
                        },
                        "DeviceKeyBoardPlugin",
        ...
        _configureCombinedView: function () {
            ...
            var reqRes = lang.clone(this.listView.requiredResources);
            this.requiredResources = lang.mixin(this.requiredResources, reqRes);
        ...
        _addActionsAndQueries: function (listView) {
            ...
            this.addChild(listView.actions);
            ...
            this.addChild(listView.queries);
        },
        ...
        setLabel: function () {
            if (this.header) {
                if (this.queries && this.queries.children) {
                    this.header.set(
                        "label",
                        this.queries.children[this.queryBaseIndex].getLabelString()
                    );
                } else {
                ...
                if (this.shouldDisplayBackButton()) {
                    ...
                    if (
                        !this.backButtonBinding1 &&
                        title &&
                        !domClass.contains(title, "mblHeadingSelectTitle")
                    ) {
                        this.backButtonBinding1 = on(
                            title,
                            touch.press,
                            lang.hitch(this, this.backButtonFunction)
                        );
                    }
                }
        ...
        buildQueryMenu: function () {
            ...
            if (this.queries && this.queries.children) {
                ...
                array.forEach(
                    ...
                    lang.hitch(this, function (query, index) {
        ...
        ensureHeaderFooterSize: function () {
            // This is needed because Dojo
        ...
        show: function () {
            ...
            if (this.ui.combinedViews) {
                ...
                if (splitterPane) {
                    ...
                    var blockUser =
                        this.isLookup() || this.showOverflow === false || this.blockUser;
                    if (this.id !== this.splitterPane.firstViewId) {
                        blockUser = true;
                    }
                    var forceBlockedToFull = false;

                    //If previous has fullsCreen true, the child should follow the same
                    if (
                        this.ui.viewHistory &&
                        this.ui.viewHistory != null &&
                        this.ui.getViewFromId(this.ui.viewHistory.slice(-1).pop().id).fullScreen &&
                        this.id !== this.ui.splitPane.leftPaneRootView
                    ) {
                    ...
                    if (this.fullScreen || (blockUser && forceBlockedToFull)) {
                        ...
                        this.paneIndex !== null &&
                        splitter.panes[this.paneIndex].open == false
                    ) {
                    ...
                    var header = this.ui.combinedViews
                        ? this.ui.getViewFromId("combined_view").header
                        : this.ui.getCurrentView().header;
            ...
            if (this.viewControl.queries) {
                $("form").attr(
                    "aria-label",
                    this.viewControl.queries.children[
                        this.viewControl.queryBaseIndex
                    ].getLabelString()
                );
            }
        },
        ...
        applyServerQueryList: function () {
            if (
                /*SystemProperties.getProperty('si.adminmode') && SystemProperties.getProperty('si.adminmode') == "true"
									&&*/ this.queries &&
                this.queries.resource
            ) {
                var previousQueryId = "";
                var previousQueryIndex = "";
                var qbData = null;
                this.queryBaseIndexResource = this.application.getResource(
                    "PlatformViewQueryResource"
                );
                if (this.queryBaseIndexResource != null) {
                    var queryBaseIndexSet = this.queryBaseIndexResource
                        .clearFilterAndSort()
                        .filter("viewid == '" + this.id + "'");
                ...
                if (
                    metdaData &&
                    metdaData.queryBasesLabel &&
                    metdaData.queryBasesLabel.length > 0
                ) {
                    var systemQueries = this.queries.children.filter(function (
                        child,
                        index,
                        array
                    ) {
                    ...
                    if (qbData) {
                        ...
                        qbData.set(
                            "queryid",
                            hasPreviousQuery ? previousQueryId : metdaData.queryBasesLabel[0].name
                        );
                    }
                }
            }
        ...
        setupFooter: function () {
            ...
            //this.baseWidget.addFixedBar(this.footer);
            this.baseWidget.addFixedBar
                ? this.baseWidget.addFixedBar(this.footer)
                : this.baseWidget.addChild(this.footer);
        },
        ...
        setupHeader: function (back) {
            ...
            this.mainHeader_Container = this.createWidget(Heading, {
                parentControl: this,
                viewControl: this,
                id: this.id + "_header_container",
                fixed: "top",
            ...
            this.backButton = new ToolbarButton({
                ...
                ui: this.ui,
                icon: this.shouldDisplayBackButton()
                    ? "header_back_OFF.png"
                    : "header_app_noBack.svg",
            ...
            if (this.shouldDisplayBackButton()) {
                this.backButtonFunction = function (e) {
                    if (
                        this.splitterPane &&
                        !this.splitterPane.splitter["maxPane"] &&
                        this.baseWidget.domNode == this.baseWidget.domNode.parentNode.firstChild
                    ) {
                        return false;
                    }
                    view.ui.back();
                };
                this.backButtonBinding = on(
                    this.backButton.baseWidget.domNode,
                    touch.press,
                    function (e) {
                ...
                domAttr.set(
                    this.backButton.baseWidget.domNode,
                    "aria-label",
                    MessageService.createStaticMessage("back").getMessage()
                );
                ...
                domAttr.set(
                    this.backButton.baseWidget.domNode,
                    "aria-label",
                    MessageService.createStaticMessage("noaction").getMessage()
                );
            }
            ...
            if (this.showOverflow) {
                ...
                on(
                    this.overFlowButton.baseWidget.domNode,
                    touch.press,
                    lang.hitch(this, function (e) {
                        try {
                            ...
                            this.logger.error(
                                "Defect 106473 - Error handling show overflow menu: " + err,
                                0
                            );
                        }
                        try {
                            ...
                            this.logger.error(
                                "Defect 106473 - Error handling e.stopPropagation(): " + err,
                                0
                            );
                        }
                    })
            ...
            this.disconnectImage.label =
                MessageService.createStaticMessage("Disconnected").getMessage();
            this.disconnectImage.className = this.disconnectImage.className + " disconnectImage";
            ...
            this.mainHeader_Container.addChild(this.header);
            ...
            this.baseWidget.addFixedBar
                ? this.baseWidget.addFixedBar(this.mainHeader_Container)
                : this.baseWidget.addChild(this.mainHeader_Container);
        },
        ...
        createToolbarButtons: function () {
            ...
            if (this.ui.actions && this.ui.actions.children && !this.uiActionsAdded) {
                if (!this.splitterPane) {
                    ...
                    if (this.ui.actions.eventHandlers) {
                        array.forEach(
                            this.ui.actions.eventHandlers,
                            lang.hitch(this.actions, function (handler) {
            ...
            array.forEach(
                allActions,
                lang.hitch(this, function (action, index) {
                    if (!action.overflow && !action.hidden && action.label) {
                        button = new ToolbarButton({
                            ...
                            action: action,
                            /* #region  Tuan-in: add css for selectall button */
                            cssClass:
                                action.label.textMsg == "Select All" ? "headerActionButtons" : "",
                            /* #endregion Tuan-out: add css for selectall button */
                        });
            ...
            //							}
            //							else{
            maxWidthPixels = document.body.clientWidth * 0.37;
            //							}
            ...
            array.forEach(
                this.potentialToolbarButtons,
                function (button, index) {
                    if (!button.overFlow) {
                        // don't hide the overflow button
                        /* #region  Tuan-in: add condition to show selectall button */
                        ...
                        //     }
                        // }
                        /* #endregion Tuan-out: add condition to show selectall button */
            ...
            if (this.overFlowButton) {
                domAttr.set(
                    this.overFlowButton.baseWidget.domNode,
                    "aria-label",
                    MessageService.createStaticMessage("more").getMessage()
                );
                domStyle.set(
                    this.overFlowButton.baseWidget.domNode,
                    "display",
                    !hasOverflow && this.hiddenButtons.length == 0 ? "none" : "block"
                );
            }
        },
            ...
            if (this == this.ui.getCombinedView() && this.splitter.panes.length > 1) {
                ...
                if (pane.showingView) {
                    subActions = subActions.concat(
                        pane.showingView.viewControl.actions.children.reverse()
                    );
                }
                //								}
                //								});
        ...
        showOverflowMenu: function (event) {
            ...
            if (this.hiddenButtons.length > 0) {
                ...
                array.forEach(
                    this.hiddenButtons,
                    lang.hitch(allItems, function (hiddenButton) {
                        var keys = Object.keys(hiddenButton);
                        var newObject = {};
                        array.forEach(
                            keys,
                            lang.hitch(newObject, function (key) {
            ...
            if (this.actions && this.actions.children && this.actions.children.length > 0) {
                ...
                array.forEach(
                    allActions,
                    lang.hitch(allItems, function (action) {
        ...
        clearToolbarButtons: function (button) {
            array.forEach(
                this.toolbarButtons,
                lang.hitch(this.header, function (button) {
                    if (!button.overFlow) {
        ...
        createMyResource: function (resourceId) {
            ...
            if (
                (this.queries && resourceId == this.queries.resource) ||
                resourceId == this.resource
            ) {
                if (this.queryBaseIndex != null) {
                    queryBase =
                        this.queries && this.queries.children
                            ? this.queries.children[this.queryBaseIndex].queryBase
                            : this.queryBase;
                } else {
                    queryBase =
                        this.queries && this.queries.children
                            ? this.queries.children[0].queryBase
                            : this.queryBase;
            ...
            if (
                //load if the resource has never been loaded
                !resourceObj ||
                //Only reload if marked and non local resource, or for local resource if we're not moving back
                (requiredResource.reload == true &&
                    (!resourceObj.isLocal() || (resourceObj.isLocal() && !this.ui.movingBack))) ||
                ...
                this.application.createResource(
                    this,
                    this.setResourceObject,
                    resourceId,
                    queryBase,
                    resourceId
                );
        ...
        setResourceObject: function (resource) {
            ...
            if (typeof requiredResource.rowHandle == "undefined") {
                requiredResource.rowHandler = resource.onChange(
                    lang.hitch(this, function (attrName, oldValue, newValue) {
        ...
        postCreate: function () {
            ...
            if (this.baseWidget.domNode.parentNode === null) return;
            ...
            if (currentView && this.viewControl) {
                ...
                if (this.application.combinedViews) {
                    ...
                    if (mainView && mainView.splitter) {
                        return array.some(
                            mainView.splitter.panes,
                            function (pane) {
                                return pane.firstViewId === currentView.id;
                            },
                            this
                        );
                    }
        ...
        changeQueryBase: function (index) {
            ...
            if (typeof index == "undefined" || index == null) {
                ...
                if (this.queryBaseIndexResource != null) {
                    ...
                    var resourceForQuery = this.queries.resource
                        ? this.queries.resource
                        : this.resource;
                    return this.application.setResourceQueryBase(
                        this,
                        resourceForQuery,
                        this.queries.children[this.queryBaseIndex].queryBase
                    );
        ...
        setQueryBaseLabel: function () {
            ...
            //This else is a solution for error founded in defect: 148778 - any questions, see the comments
                ...
                !this.hasErroredRecords &&
                this.queryBase == "__errored__" &&
                this.queries.resource
            ) {
                this.setQueryErrorMarker(this.hasErroredRecords);
                var _contErroredPromise = this.application.modelService.countErrored(
                    this.queries.resource
                );
                ...
                _contErroredPromise
                    ...
                    .otherwise(
                        lang.hitch(this, function (error) {
                            this.logger.log(
                                "[platform.ui.control.View] error on trying to count the number of registers with errors to build the query menu: " +
                                    error
                            );
                        })
                    );
        ...
        bindQueryMenu: function () {
            ...
            if (
                title &&
                domClass.contains(title, "mblHeadingSelectTitle") &&
                !this.querySelectorListener
            ) {
                this.querySelectorListener = on(
                    title,
                    touch.press,
                    lang.hitch(this, function (e) {
                        if (!this._isQuerySelectorMenuShowing()) {
                            this.ui.showMenu(
                                ...
                                e,
                                this.header.domNode.children[queryDropdownIndex],
                                this.queryMenuItems,
                                "overflowMenu",
                                null,
                                this
        ...
        validate: function () {
            if (
                ...
                (!this.exceptionFields || this.application.getObjectSize(this.exceptionFields) == 0)
            ) {
                return true;
            }
            var emptyField = null;
            ...
            if (
                dojo.some(this.requiredFields, function (field) {
                ...
                this.stopForValidation(
                    emptyField,
                    this.application.requiredFieldLabel.getMessage()
                );
                return false;
            }
            ...
            if (this.application.getObjectSize(this.exceptionFields) > 0) {
                ...
                if (errorField) {
                    ...
                    if (!control || !control.getResource()["validateErrorShown"]) {
                        ...
                        this.stopForValidation(
                            errorField.parentControl,
                            this.application.invalidFieldLabel.getMessage() +
                                "<br>" +
                                this.exceptionFields[
                                    Object.keys(this.exceptionFields)[0]
                                ].getMessage()
                        );
                    }
                }
        ...
        stopForValidation: function (control, message) {
            this.ui.showMessage(message, function () {
                ...
                on.once(
                    control.baseWidget.domNode,
                    "keypress",
                    lang.hitch(control, function () {
        ...
        scrollIntoView: function (control, top) {
            ...
            if (
                nodeBox.y < 0 ||
                nodeBox.y > domGeometry.getMarginBox(control.viewControl.baseWidget.domNode).h
            ) {
        ...
        scrollDomNodeIntoView: function (domNode) {
            ...
            if (footerNodeBox) obstructedByFooter = nodeBox.y + nodeBox.h > footerNodeBox.y;
            var offScreen =
                nodeBox.y < 0 || nodeBox.y > domGeometry.getMarginBox(this.baseWidget.domNode).h;
        ...
        manageException: function (control, error) {
            ...
            if (error) {
                if (
                    error instanceof PlatformRuntimeException &&
                    !(error instanceof PlatformRuntimeWarning)
                ) {
        ...
        getCurrentQueryBaseLabel: function () {
            if (
                this.queries &&
                this.queries.children &&
                this.queries.children[this.queryBaseIndex]
            ) {
        ...
        setQueryErrorMarker: function (hasErrors) {
            if (this.ui.combinedViews && this.splitterPane && this.splitterPane.paneNum == 0) {
                //must call on combined_view
            ...
            if (this.queries) {
                ...
                var queryDropdownIndex = 1;
                var view = this.listView ? this.listView : this;
                var queryLabel = XSSSanitizer.sanitizeValue(
                    view.queries.children[view.queryBaseIndex].getLabelString()
                );
                this.header.domNode.children[queryDropdownIndex].innerHTML = queryLabel;
                domClass.add(
                    this.header.domNode.children[queryDropdownIndex],
                    "mblHeadingSelectTitle"
                );
                this.header.domNode.children[queryDropdownIndex].setAttribute("role", "button"); //tpl-domAttr not defined here?
                this.header.domNode.children[queryDropdownIndex].setAttribute(
                    "aria-label",
                    queryLabel
                );
        ...
        buildFooterButtons: function () {
            if (this.footer && this.footerButtons && this.footerButtons.length > 0) {
                this.destroyFooter();
                var numbuttons = this.visibleButtonCount
                    ? parseInt(this.visibleButtonCount, 10)
        ...
        destroyFooter: function () {
            if (this.footer) {
                ...
                if (this.footer.destroyDescendants) this.footer.destroyDescendants();
            }
        },
        ...
        suppressMinorVertical: function (x, y) {
            //movement offsets (could be negative)
            x = Math.abs(x);
            y = Math.abs(y);
            if (y < x) {
                return false;
        ...
        initializeAccessbleScrollView: function () {
            ...
            var scroll_totalHeight = domGeometry.getMarginBox(
                this.scrollWidget.domNode.firstChild
            ).h;
        ...
        setupNotificationModal: function () {
            this.pushContainer = this.createWidget(Container, {
                id: this.getId() + "_pushContainer",
                ui: this.ui,
                parentControl: this,
                application: this.application,
                class: "push-notification-modal-container",
            });

            this.notificationWidget = new NotificationModalWdget({
                openAction: this.openRecord,
                nextAction: this.nextPushRecord,
                dismissAction: this.dismissRecord,
                closeAction: this.closeNotificationModal,
                parentControl: this,
            }).placeAt(this.pushContainer);

            this.mainHeader_Container.addChild(this.pushContainer);

            var nt_ico =
                "<span style='visibility:hidden; vertical-align: middle; box-shadow: none;border: none; margin-left: 19px !important;' role='button' class='mblToolBarButton mblColorDefault mblToolBarButtonBody mblToolBarButtonLightIcon' tabindex='0'>" +
                "<svg xmlns='http://www.w3.org/2000/svg' width='32px' height='32px' viewBox='" +
                "0 0 32 32'><path d='M28.7 20.3L26 17.6V13a10.07 10.07 0 0 0-9-10V1h-2v2a10.15 10.15 0 0 0-9 10v4.6l-2.7 2.7a.91.91 0 0 0-.3.7v3a.94.94 0 0 0 1 1h7a5 5 0 0 0 10 0h7a.94.94 0 0 0 1-1v-3a.91.91 0 0 0-.3-.7zM16 28a3 3 0 0 1-3-3h6a3 3 0 0 1-3 3zm11-5H5v-1.6l2.7-2.7A.91.91 0 0 0 8 18v-5a8 8 0 0 1 16 0v5a.91.91 0 0 0 .3.7l2.7 2.7z'" +
                "fill='rgb(214, 214, 214)'/>" +
                "<circle class='push-icon-circle' cx='23' cy='9.5' r='8.5'/>" +
                "<text id='" +
                this.getId() +
                "_push_notification_number' class='push-icon-number' x='23' y='14' text-anchor='middle'>99</text></svg></div></span>";

            this.notification_icon = domConstruct.place(
                nt_ico,
                this.disconnectImage.baseWidget.domNode,
                "before"
            );
        ...
        showNotificationIcon: function (resourceSet) {
            push_icon_display = true;
            var notificationResourceSet = null;
            if (!resourceSet) {
                notificationResourceSet = this.application.getResource(
                    "PlatformTempPushNotification"
                );
            } else {
                notificationResourceSet = resourceSet;
            }

            domStyle.set(this.notification_icon, "visibility", "visible");
            this.updateNotificationNumber(resourceSet);
        },

        updateNotificationNumber: function (resourceSet) {
            var domNotificationNumber = dom.byId(this.getId() + "_push_notification_number");
            var resCount = resourceSet.count();
            domNotificationNumber.textContent = resCount;
            if (
                WL.Client.getEnvironment() == WL.Environment.IPHONE ||
                WL.Client.getEnvironment() == WL.Environment.IPAD
            ) {
                WL.Badge.setNumber(resCount);
            }
        },

        hideNotificationIcon: function () {
            push_icon_display = false;
            if (this.notification_icon) {
                domStyle.set(this.notification_icon, "visibility", "hidden");
            }
            if (
                WL.Client.getEnvironment() == WL.Environment.IPHONE ||
                WL.Client.getEnvironment() == WL.Environment.IPAD
        ...
        showNotificationModal: function (resourceSet) {
            var disableOpenButton = false;
            push_modal_display = true;
            var notificationResourceSet = null;
            if (!resourceSet) {
                notificationResourceSet = this.application.getResource(
                    "PlatformTempPushNotification"
            ...
            if (
                this.id !== this.ui._getPrimaryViewID() ||
                notificationResourceSet.getCurrentRecord().get("msgType") === "alert"
            )
                disableOpenButton = true;

            this.notificationWidget.show(notificationResourceSet, disableOpenButton);

            Logger.trace(
                "[INFX89D] DomStyle Visibility ------------" +
                    domClass.contains(
                        this.baseWidget.domNode.children[0],
                        "push-notification-scrollview-active"
                    )
            );
            if (
                !domClass.contains(
                ...
                Logger.trace("[INFX89D] Entering to show visibility");
                domClass.add(this.mainHeader_Container.domNode, "push-notification-header-active");
                domClass.add(this.pushContainer.domNode, "push-notification-modal-active");
                domClass.add(
                    this.baseWidget.domNode.children[0],
                    "push-notification-scrollview-active"
                );
        ...
        hideNotificationModal: function () {
            push_modal_display = false;
            if (this.mainHeader_Container && this.pushContainer && this.baseWidget) {
                domClass.remove(
                    this.mainHeader_Container.domNode,
                    "push-notification-header-active"
                );
                domClass.remove(this.pushContainer.domNode, "push-notification-modal-active");
                domClass.remove(
                    this.baseWidget.domNode.children[0],
                    "push-notification-scrollview-active"
                );
                //domClass.add(this.baseWidget.domNode.children[0],'push-notification-scrollview-inactive');
            }
        },
        ...
        openRecord: function (eventContext) {
            ...
            var rs = eventContext.parentControl.application
                .getResource("PlatformTempPushNotification")
                .getCurrentRecord();
            ...
            var self = this;
            ModelService.byRef(resource, recordId)
                .then(function (modelDataSet) {
                    if (
                        WL.Client.getEnvironment() == WL.Environment.IPHONE ||
                        WL.Client.getEnvironment() == WL.Environment.IPAD
                    ) {
                    ...
                    Logger.trace("INFXPN_405: ModelDataset count: " + modelDataSet.data.length);
                    if (modelDataSet.data.length > 0) {
                        Logger.trace(
                            "INFXPN_405: ModelDataset count: " + modelDataSet.data[0].wonum
                        );
                    }
                    if (modelDataSet.data.length == 0) {
                        //Logger.trace("INFXPN_467: No record found");
                        eventContext.parentControl.application.hideBusy();
                        eventContext.parentControl.application.ui.show(
                            "Platform.NoRecordFoundDialog"
                        );
                    } else {
                        //Logger.trace("INFXPN_103: viewHistory after: ModelServiceRef before:manipulating history" + JSON.stringify(this.ui.viewHistory));
                        while (
                            eventContext.parentControl.ui.viewHistory[
                                eventContext.parentControl.ui.viewHistory.length - 1
                            ].id != eventContext.parentControl.application.ui.defaultView
                        ) {
                            //Logger.trace("INFXPN_104: viewHistory in:while" + JSON.stringify(this.ui.viewHistory));
                            eventContext.parentControl.ui.viewHistory.pop();
                        }

                        eventContext.parentControl.application.ui
                            .getViewFromId(eventContext.parentControl.application.ui.defaultView)
                            .onlyChangeQueryBaseIndex(PlatformConstants.SEARCH_RESULT_QUERYBASE);
                        modelDataSet._queryBases = PlatformConstants.SEARCH_RESULT_QUERYBASE;
                        eventContext.parentControl.application.addResource(modelDataSet);

                        //Logger.trace("INFXPN_106: transitionTo" + transitionTo);
                        eventContext.parentControl.application.ui.show(transitionTo);
                    }
                })
                .otherwise(function (error) {
                    Logger.trace("INFXPN_116:" + JSON.stringify(error));
                })
                .always(function () {
                    var dismissRec = lang.hitch(self, eventContext.parentControl.dismissRecord);
                    dismissRec(eventContext);
                    eventContext.parentControl.application.hideBusy();
                });
        },

        dismissRecord: function (widget) {
            var disableOpenButton = false;
            var rs = widget.parentControl.application.getResource("PlatformTempPushNotification");
            var curr = rs.getCurrentRecord();

            rs._cleanupRecord(curr);
            curr.deleteLocal();
            var self = this;
            widget.parentControl.application.showBusy();
            ModelService.save(rs)
                .then(function () {
                    if (rs.count() === 0) {
                        widget.parentControl.viewControl.hideNotificationModal();
                        widget.parentControl.viewControl.hideNotificationIcon();
                        widget.parentControl.application.hideBusy();
                        return;
                    } else if (rs.count() === 1) {
                    ...
                    widget.parentControl.viewControl.updateNotificationNumber(rs);

                    if (
                        self.parentControl.id !== self.parentControl.ui._getPrimaryViewID() ||
                        rs.getCurrentRecord().get("msgType") === "alert"
                    )
                        disableOpenButton = true;

                    widget.show(rs, disableOpenButton);
                    widget.parentControl.application.hideBusy();
                })
                .otherwise(function (error) {
                    Logger.trace(
                        "Notification Dismiss couldn't save the function. Error: " + error
                    );
                    widget.parentControl.application.hideBusy();
                });
        },

        nextPushRecord: function () {
            var disableOpenButton = false;
            var rs = this.parentControl.application.getResource("PlatformTempPushNotification");
            if (rs.hasNext()) {
                rs.next();
            } else {
                rs.setCurrentIndex(0);
            ...
            if (
                this.parentControl.id !== this.parentControl.ui._getPrimaryViewID() ||
                rs.getCurrentRecord().get("msgType") === "alert"
            )
                disableOpenButton = true;

            this.show(rs, disableOpenButton);
        },

        closeNotificationModal: function () {
            this.parentControl.hideNotificationModal();
        },
    });
});
