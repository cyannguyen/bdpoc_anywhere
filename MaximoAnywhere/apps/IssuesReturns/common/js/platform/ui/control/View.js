...
define("platform/ui/control/View", [
        "dojo/_base/declare",
		  ...
		  "platform/util/XSSSanitizer"],//TODO: WorkOfflineHandler here is a workaround just for be able to use ios 12, need fix it properly.
		function(declare, Actions, ContainerControlBase, ScrollableView, View, FixedSplitter, dojoButton, Heading, ToolbarButton, Button, Query, Image, bookmarkable, domConstruct,
				...
				PlatformRuntimeWarning, SystemProperties, ResourceMetadataContext, topic, Queries, Query, Actions, Action, ConnectivityChecker, dijitFocus, Container, NotificationModalWdget,Logger,
				WorkOfflineHandler, XSSSanitizer) { //TODO: WorkOfflineHandler here is a workaround just for be able to use ios 12, need fix it properly.
				...
				return declare([ ContainerControlBase, OrientationEventListenerMixin, WorkOfflineHandler], {//TODO: WorkOfflineHandler here is a workaround just for be able to use ios 12, need fix it properly.
						id : null,
						label : '',
						baseWidget : null,
						mainView:null,
						...
						sync: function(eventContext){
							if(this && this.application && this.application['platform.handlers.WorkOfflineHandler']){
								var workOfflineHandlerObject = this.application['platform.handlers.WorkOfflineHandler'];
								if(workOfflineHandlerObject){
									...
									this.logger.trace("View.js: calling sync from platform.handlers.WorkOfflineHandler");
								}
							}
						},
						...
						build : function() {
							...
							var isCombinedViewChildrenTransitioning = function(thisObj){
								return (thisObj.id === thisObj.ui.splitPane.leftPaneRootView && thisObj.ui.getCurrentView().id === thisObj.ui.splitPane.rightPaneRootView) ||
									(thisObj.id === thisObj.ui.splitPane.rightPaneRootView && thisObj.ui.getCurrentView().id === thisObj.ui.splitPane.leftPaneRootView);
							}
							...
							var afterTransitionFunc = function(lt){
								...
								if(lt.ui.combinedViews){ 
									if(lt.ui.getViewFromId(lt.ui.transitionInfo.id).viewControl.isLookup())
										return
								}
								else {
								...
								var self  = lt;
								if(lt.application.isFeatureEnabled('pushnotification.enabled') && currView.movedFrom === 'platform.LoginView' && currView.id === lt.ui._getPrimaryViewID()){
							...
							on(this.baseWidget, 'BeforeTransitionIn', lang.hitch(this, function(moveTo, dir, transition, context, method) {
								...
								if(!this.transitionInOutFLag && this.ui.combinedViews && this.id !== "platform.LoginView"){
									...
									if(this.ui.getShowingView().id !== "combined_view" && !isCombinedViewChildrenTransitioning(this)) {
								...
								if(WL.Client.getEnvironment() == WL.Environment.IPHONE || WL.Client.getEnvironment() == WL.Environment.IPAD){ 
									...
									var wasOffline = (localStorage.getItem("wasOffline") && (localStorage.getItem("wasOffline") == "true" || localStorage.getItem("wasOffline") == "false")) ?  localStorage.getItem("wasOffline") : "false";                  
								...
								if (appName && SystemProperties.getProperty('demoMode.enabled')) {
									this.ui.showSnackBarMessage(MessageService.createResolvedMessage('DemoModeIndicator',[appName]));
								}



								if(this.application.isFeatureEnabled('pushnotification.enabled') && this.id !== 'platform.LoginView' && this.id !== PlatformConstants.EMPTY_VIEW){
							...
							on(this.baseWidget, 'AfterTransitionIn', lang.hitch(this, function(moveTo, dir, transition, context, method) {
								...
								if(this.application.isFeatureEnabled('pushnotification.enabled') && currView.movedFrom === 'platform.LoginView' && currView.id === this.ui._getPrimaryViewID()){
									ModelService.all('PlatformTempPushNotification', null).then(function(recordSet){
										recordSet.resourceID = 'PlatformTempPushNotification';
										self.application.addResource(recordSet);
										if(recordSet.count() !== 0){
								...
								if(this.ui.combinedViews && this.id === this.ui.splitPane.leftPaneRootView){
						        	...
						        	if(listChildren && listChildren.length > 0){
						        		this.ui.removeOverlay(this.ui.getViewFromId(this.ui.splitPane.rightPaneRootView).baseWidget);
						        		var listItem = this.lists[0].listWidget.getChildren()[idx];
						        		this.lists[0].listWidget.setCurrentRecord(listItem);
						        	}else{//There are no rows in the left pane
						        		this.ui.addOverlay(this.ui.getViewFromId(this.ui.splitPane.rightPaneRootView).baseWidget);
						...
						hideSoftKeyBoard : function() {
							if (WL.Client.getEnvironment() == WL.Environment.ANDROID) {
								...
								setTimeout(() => {
									domStyle.set(dom.byId(WL.application.ui.transitionInfo.id), "height", (document.documentElement.clientHeight) + "px");
								}, 100);
							} else {
								// Hiding the keyboard
...
// }						
							...
							if(this.ui.combinedViews){
								...
								if(splitterPane){
									...
									var blockUser = this.isLookup() || this.showOverflow===false || this.blockUser;
									...
									if(this.ui.viewHistory && this.ui.viewHistory != null && this.ui.getViewFromId(this.ui.viewHistory.slice(-1).pop().id).fullScreen && this.id !== this.ui.splitPane.leftPaneRootView){
						...
						setupHeader : function(back) {
							...
							this.backButton = new ToolbarButton({
								...
								ui : this.ui,
								icon : this.shouldDisplayBackButton() ? 'header_back_OFF.png' : 'header_app_noBack.svg',
							...
							if(this.application.isFeatureEnabled('pushnotification.enabled') && this.id !== 'platform.LoginView'){
								this.setupNotificationModal();
							}
							
							//this.baseWidget.addFixedBar(this.header);
							this.baseWidget.addFixedBar?this.baseWidget.addFixedBar(this.mainHeader_Container):this.baseWidget.addChild(this.mainHeader_Container);
...
//								});
						...
						setQueryErrorMarker : function(hasErrors) {
							...
							if (this.queries) {
								...
								var queryDropdownIndex = 1;
								var view = this.listView?this.listView:this;
								var queryLabel = XSSSanitizer.sanitizeValue( view.queries.children[view.queryBaseIndex].getLabelString());
    							...
    							this.header.domNode.children[queryDropdownIndex].setAttribute('aria-label',queryLabel);
						...
						destroyFooter : function() {
							...
							if (this.footer ) {
								if (this.footer && this.footerButtons && this.footerButtons.length > 0) {
									...
									if(this.footer.destroyDescendants)
						...
						setupNotificationModal: function(){
							...
							var nt_ico =  "<span style='visibility:hidden; vertical-align: middle; box-shadow: none;border: none; margin-left: 19px !important;' role='button' class='mblToolBarButton mblColorDefault mblToolBarButtonBody mblToolBarButtonLightIcon' tabindex='0'>" + 
									"<svg xmlns='http:\/\/www.w3.org\/2000\/svg' width='32px' height='32px' viewBox='"+"0 0 32 32'><path d='M28.7 20.3L26 17.6V13a10.07 10.07 0 0 0-9-10V1h-2v2a10.15 10.15 0 0 0-9 10v4.6l-2.7 2.7a.91.91 0 0 0-.3.7v3a.94.94 0 0 0 1 1h7a5 5 0 0 0 10 0h7a.94.94 0 0 0 1-1v-3a.91.91 0 0 0-.3-.7zM16 28a3 3 0 0 1-3-3h6a3 3 0 0 1-3 3zm11-5H5v-1.6l2.7-2.7A.91.91 0 0 0 8 18v-5a8 8 0 0 1 16 0v5a.91.91 0 0 0 .3.7l2.7 2.7z'"+ 
									"fill='rgb(214, 214, 214)'\/>" +
									"<circle class='push-icon-circle' cx='23' cy='9.5' r='8.5'\/>"+
									"<text id='"+ this.getId() +"_push_notification_number' class='push-icon-number' x='23' y='14' text-anchor='middle'>99<\/text><\/svg><\/div><\/span>"
							
							this.notification_icon = domConstruct.place(nt_ico, this.disconnectImage.baseWidget.domNode, 'before');

							on(this.notification_icon, touch.press, lang.hitch(this, function(e) {
									try {
						...
						showNotificationIcon: function(resourceSet){
							...
							if(!resourceSet){
								notificationResourceSet = this.application.getResource('PlatformTempPushNotification')
							}
							else{
						...
						showNotificationModal: function(resourceSet){
							...
							if(this.id !== this.ui._getPrimaryViewID() || notificationResourceSet.getCurrentRecord().get('msgType') === 'alert')
								disableOpenButton = true;
							
							this.notificationWidget.show(notificationResourceSet, disableOpenButton);
							
							Logger.trace("[INFX89D] DomStyle Visibility ------------" + domClass.contains(this.baseWidget.domNode.children[0], 'push-notification-scrollview-active'));
							if(!domClass.contains(this.baseWidget.domNode.children[0], 'push-notification-scrollview-active')){
								...
								domClass.add(this.baseWidget.domNode.children[0], 'push-notification-scrollview-active');
							}
						...
						hideNotificationModal: function(){
							...
							if(this.mainHeader_Container && this.pushContainer && this.baseWidget){
								domClass.remove(this.mainHeader_Container.domNode, 'push-notification-header-active');
								domClass.remove(this.pushContainer.domNode, 'push-notification-modal-active');
								domClass.remove(this.baseWidget.domNode.children[0], 'push-notification-scrollview-active');
									...
									ModelService.byRef(resource, recordId).then(function(modelDataSet){
										if (WL.Client.getEnvironment() == WL.Environment.IPHONE || WL.Client.getEnvironment() == WL.Environment.IPAD) {
											...
											if(modelDataSet.data.length>0){
												Logger.trace("INFXPN_405: ModelDataset count: " + modelDataSet.data[0].wonum)
											}
											if(modelDataSet.data.length==0){
												
												//Logger.trace("INFXPN_467: No record found");
												eventContext.parentControl.application.hideBusy();
												eventContext.parentControl.application.ui.show("Platform.NoRecordFoundDialog");
											}else{
									
										
												//Logger.trace("INFXPN_103: viewHistory after: ModelServiceRef before:manipulating history" + JSON.stringify(this.ui.viewHistory));
												while(eventContext.parentControl.ui.viewHistory[eventContext.parentControl.ui.viewHistory.length-1].id != eventContext.parentControl.application.ui.defaultView){
												...
												eventContext.parentControl.application.ui.getViewFromId(eventContext.parentControl.application.ui.defaultView).onlyChangeQueryBaseIndex(PlatformConstants.SEARCH_RESULT_QUERYBASE);
						...
						dismissRecord: function(widget){
							...
							ModelService.save(rs).then(function(){
								...
								if(self.parentControl.id !== self.parentControl.ui._getPrimaryViewID() || rs.getCurrentRecord().get('msgType') === 'alert')
									disableOpenButton = true;

								widget.show(rs, disableOpenButton);
								widget.parentControl.application.hideBusy();
							}).otherwise(function(error){
								Logger.trace("Notification Dismiss couldn't save the function. Error: " + error);
								widget.parentControl.application.hideBusy();
							});
						...
						nextPushRecord: function(){
							...
							if(this.parentControl.id !== this.parentControl.ui._getPrimaryViewID() || rs.getCurrentRecord().get('msgType') === 'alert')
								disableOpenButton = true;
